...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate NTP accuracy by synthetic hierarchy boundary type.")
    ...
    parser.add_argument("--output_path", type=str, default="../experiments/capacity_boundary_eval.json")
...
def evaluate_run(args, run):
    ...
    for mod in range(args.synthetic_block_size):
        counters[f"position_mod_{mod}"] = new_counter()
    ...
    for start in range(0, args.num_samples, args.batch_size):
        ...
        positions = torch.arange(source.shape[1])[None, :].expand_as(source)
        ...
        masks = {
            ...
            "inside_high": valid & high_same,
        }
        for mod in range(args.synthetic_block_size):
            masks[f"position_mod_{mod}"] = valid & ((positions % args.synthetic_block_size) == mod)
        for name, mask in masks.items():
            update(counters[name], per_token_loss, pred, target, mask)
...
def main():
    ...
    results = {
        "config": vars(args),
        "runs": [evaluate_run(args, run) for run in runs],
    }
    ...
    for row in results["runs"]:
        ...
        print(
            row["run"],
            f"overall={metrics['overall']['accuracy']:.4f}/{metrics['overall']['loss']:.4f}",
            f"inside={metrics['inside_local']['accuracy']:.4f}",
            f"local_boundary={metrics['local_boundary']['accuracy']:.4f}",
            f"local_boundary_not_high={metrics['local_boundary_not_high']['accuracy']:.4f}",
            f"high_boundary={metrics['high_boundary']['accuracy']:.4f}",
        )
