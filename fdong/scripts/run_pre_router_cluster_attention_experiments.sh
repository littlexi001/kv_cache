...
export PRE_ROUTER_CONTROLS_ATTENTION="${PRE_ROUTER_CONTROLS_ATTENTION:-true}"  # true/false

export ATTENTION_ROUTER_LOSS_TYPE="${ATTENTION_ROUTER_LOSS_TYPE:-kl}"  # kl/pairwise/topk_logits; ignored when weight=0
...
run_one() {
  local suffix="$1"
  local lb_weight="$2"
  local run_name="pre-router-cluster-attn-${SYNTHETIC_SAMPLING_DISTRIBUTION}-${PRE_ROUTER_INPUT}-${suffix}"

  echo "==== training ${run_name} ===="
  MOE_LOAD_BALANCE_LOSS_WEIGHT="${lb_weight}" \
  CKPT_DIR="../checkpoints/${run_name}" \
  bash single_thread_debug.sh > "../logs/${run_name}.log" 2>&1
}

run_one "ntp-only" "0.0"
run_one "lb0.01" "0.01"

echo "==== done ===="
echo "logs: ../logs/pre-router-cluster-attn-${SYNTHETIC_SAMPLING_DISTRIBUTION}-${PRE_ROUTER_INPUT}-*.log"
echo "checkpoints: ../checkpoints/pre-router-cluster-attn-${SYNTHETIC_SAMPLING_DISTRIBUTION}-${PRE_ROUTER_INPUT}-*"
