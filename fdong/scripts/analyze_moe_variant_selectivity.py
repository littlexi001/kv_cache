...
def build_config(args, runtime_config):
    ...
    config.moe_spectral_band_dims = runtime_config.get("moe_spectral_band_dims", None)
    config.moe_spectral_num_experts_per_band = runtime_config.get("moe_spectral_num_experts_per_band", None)
    config.moe_spectral_topk_per_band = runtime_config.get("moe_spectral_topk_per_band", None)
    config.moe_spectral_intermediate_sizes = runtime_config.get("moe_spectral_intermediate_sizes", None)
    config.moe_spectral_update_interval = int(runtime_config.get("moe_spectral_update_interval", 100))
    config.moe_spectral_warmup_steps = int(runtime_config.get("moe_spectral_warmup_steps", 100))
    config.moe_spectral_sample_size = int(runtime_config.get("moe_spectral_sample_size", 4096))
    config.moe_spectral_basis_momentum = float(runtime_config.get("moe_spectral_basis_momentum", 0.0))
    config.moe_spectral_include_top_in_router = bool(runtime_config.get("moe_spectral_include_top_in_router", True))
