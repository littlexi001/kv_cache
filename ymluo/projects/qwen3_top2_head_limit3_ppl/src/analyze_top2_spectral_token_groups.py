...
_ACTIVE_COLLECTOR: "SpectralTokenCollector | None" = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze whether true top-fraction attention tokens and evidence token groups "
            "concentrate in low-rank singular directions of per-layer/head K-cache spaces."
        )
    )
    ...
    parser.add_argument("--seed", type=int, default=2026063001)
    ...
    parser.add_argument(
        "--max_query_tokens_per_task",
        type=int,
        default=2,
        help="Sample this many final query tokens from each task. Use <=0 for all query tokens.",
    )
    parser.add_argument(
        "--max_tokens_per_group_per_row",
        type=int,
        default=32,
        help="Cap non-top2 group tokens per layer/head/query row to keep SVD-side statistics light.",
    )
    ...
    def _observe_head(
        ...
        *,
        ...
        for cutoff in cutoffs:
            svd_values[f"sv_energy_top{cutoff}"] = float(energy_cdf[cutoff - 1].item())
    ...
    def _add_group_event(
        ...
        *,
        ...
        for cutoff in cutoffs:
            ...
            values[f"q_energy_top{cutoff}"] = q_top
            values[f"token_energy_top{cutoff}"] = k_top
...
def _spectral_eager_attention_forward(
...
def install_qwen3_attention_patch() -> None:
    ...
    if _ORIGINAL_EAGER_ATTENTION_FORWARD is None:
        ...
        setattr(modeling_qwen3, "eager_attention_forward", _spectral_eager_attention_forward)
        if hasattr(modeling_qwen3, "ALL_ATTENTION_FUNCTIONS"):
            modeling_qwen3.ALL_ATTENTION_FUNCTIONS["eager"] = _spectral_eager_attention_forward


@contextmanager
def active_collector(collector: SpectralTokenCollector):
...
def run_query_eval(
    ...
    chunk_size: int,
    input_device: torch.device,
    collector: SpectralTokenCollector,
) -> None:
...
def main() -> None:
    ...
    for cutoff in rank_cutoffs:
        group_metric_fields += [
            f"q_energy_top{cutoff}",
            f"token_energy_top{cutoff}",
            f"abs_qk_contrib_top{cutoff}",
            f"signed_qk_dot_top{cutoff}",
            f"cosine_top{cutoff}",
        ]
    svd_metric_fields = ["effective_rank", "rank50", "rank80", "rank90", "rank95", "rank99"]
    svd_metric_fields += [f"sv_energy_top{cutoff}" for cutoff in rank_cutoffs]

    write_csv(
        output_dir / "group_spectral_stats.csv",
        collector.group_rows(group_metric_fields),
        ["scope", "group", "layer", "head", "cases"] + group_metric_fields,
    )
    write_csv(
        output_dir / "svd_energy_stats.csv",
    ...
    summary = {
        ...
        "resolved": {
            ...
            "seconds": seconds,
            "metric_notes": {
                "sv_energy_topK": "CDF of squared singular values of centered K history matrix.",
                "token_energy_topK": "Fraction of a token K vector squared norm captured by first K right-singular directions.",
                "q_energy_topK": "Same projection-energy fraction for the sampled query vector.",
                "abs_qk_contrib_topK": "Fraction of absolute per-direction q*k dot-product contribution in first K singular directions.",
                "cosine_topK": "Cosine between query and token vectors after projecting to first K singular directions.",
            },
        },
        "paths": {
            "group_spectral_stats": str(output_dir / "group_spectral_stats.csv"),
            "svd_energy_stats": str(output_dir / "svd_energy_stats.csv"),
