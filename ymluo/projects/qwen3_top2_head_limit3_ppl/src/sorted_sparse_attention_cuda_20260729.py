...
torch::Tensor sorted_sparse_attention_split_cuda(
    torch::Tensor query,
    torch::Tensor key,
    torch::Tensor value,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      "forward",
      &sorted_sparse_attention_split_cuda,
      "Split sparse attention with block-local candidate sorting");
}
"""
...
__global__ void sorted_sparse_attention_split_kernel(
  ...
  int64_t* sorted_indices = reinterpret_cast<int64_t*>(
      reduction + aligned_float_count);
  ...
  for (int local = tid; local < sort_count; local += blockDim.x) {
    int64_t token = std::numeric_limits<int64_t>::max();
    if (local < local_count) {
      int selected = start + local;
      token = selected == candidate_count
          ? static_cast<int64_t>(key_count - 1)
          : idx_row[selected];
    }
    sorted_indices[local] = token;
  }
  __syncthreads();
  for (int size = 2; size <= sort_count; size <<= 1) {
    for (int stride = size >> 1; stride > 0; stride >>= 1) {
      for (int local = tid; local < sort_count;
           local += blockDim.x) {
        int other = local ^ stride;
        if (other > local) {
          bool ascending = (local & size) == 0;
          int64_t left = sorted_indices[local];
          int64_t right = sorted_indices[other];
          if ((left > right) == ascending) {
            sorted_indices[local] = right;
            sorted_indices[other] = left;
          }
        }
      }
      __syncthreads();
  ...
  for (int local = tid; local < local_count; local += blockDim.x) {
    int64_t token = sorted_indices[local];
...
torch::Tensor sorted_sparse_attention_split_cuda(
  ...
  int sort_capacity = 1;
  while (sort_capacity < max_local_count) {
    sort_capacity <<= 1;
  }
  int float_count = threads + max_local_count;
  int aligned_float_count = (float_count + 1) & ~1;
  size_t shared_bytes =
      static_cast<size_t>(aligned_float_count) * sizeof(float)
      + static_cast<size_t>(sort_capacity) * sizeof(int64_t);
  TORCH_CHECK(
      shared_bytes <= 48 * 1024,
      "sorted sparse attention exceeds 48 KiB shared memory");

  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half,
      at::ScalarType::BFloat16,
      query_c.scalar_type(),
      "sorted_sparse_attention_split",
      [&] {
        sorted_sparse_attention_split_kernel<scalar_t><<<
...
def load_extension() -> Any:
    return load_inline(
        name="qksieve_sorted_sparse_attention_20260729_v2",
