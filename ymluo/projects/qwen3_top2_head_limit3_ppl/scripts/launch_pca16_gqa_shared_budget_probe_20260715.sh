...
RUN_ROOT="${RUN_ROOT:-${ROOT}/results/20260715_pca16_gqa_shared_budget_probe_32k_w0}"
mkdir -p "${RUN_ROOT}"

topics=(sports medicine sports medicine sports medicine)
fractions=(0.02 0.02 0.03 0.03 0.04 0.04)
gpus=(${GPUS:-0 4 5 6 7 3})
for slot in ${SLOTS:-0 1 2 3 4 5}; do
  gpu="${gpus[$slot]}"
  topic="${topics[$slot]}"
  fraction="${fractions[$slot]}"
  name="${topic}_w0_pca16_shared_mean_f${fraction}"
  nohup env \
