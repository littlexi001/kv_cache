...
if [[ "${#gpu_list[@]}" -lt "${#LENGTHS[@]}" ]]; then
  echo "Need at least ${#LENGTHS[@]} GPUs" >&2
  exit 1
fi
mkdir -p "${RUN_ROOT}/logs"
rm -f "${RUN_ROOT}/FAILED" "${RUN_ROOT}/ALL_COMPLETE"
{
  echo "purpose=independent CUDA-event stage timings"
  echo "model=${MODEL}"
  echo "lengths=${LENGTHS[*]}"
  echo "eval_tokens=${EVAL_TOKENS}"
  echo "variant=${VARIANT}"
  echo "QKSIEVE_PROFILE_STAGES=1"
  sha256sum \
...
for index in "${!LENGTHS[@]}"; do
  run_length "${LENGTHS[$index]}" "${gpu_list[$index]}" &
  pids+=("$!")
done
status=0
for pid in "${pids[@]}"; do
