...
OUTPUT="${OUTPUT:-$ROOT/results/20260727_fractional_tail_spectral_rate}"
CENTER_KEYS="${CENTER_KEYS:-0}"
LOGS="$OUTPUT/logs"
...
cd "$ROOT"
center_args=()
if [[ "$CENTER_KEYS" == "1" ]]; then
  center_args+=(--center_keys)
fi
...
run_case() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" -u \
    src/analyze_fractional_tail_spectral_rate_20260727.py \
    --trace_path "$trace" \
    --output_dir "$output" \
    --label "$label" \
    --device cuda \
