...
STAMP="${STAMP:-20260712_after_pareto055_v399}"
LABEL="${LABEL:-v399_cost_router055_after_pareto}"
POLICY="${POLICY:-configs/riskkv_task_policy_v399_cost_router055_after_pareto_20260712.json}"
...
echo "TRAIN v399 cost-aware router kv<=5.5% $(date -Is)"
"$PY" scripts/train_cost_aware_router_v397_after_pareto_20260712.py \
  --output-dir outputs/riskkv_v19_cost_router_v399_after_pareto055_20260712 \
  --config-out "$POLICY" \
  --kv-limit 0.055 \
  --cal-min-gain -0.001 \
  --test-min-gain 0.0
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v399_cost_router055_after_pareto_20260712_after_pareto055_v399_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v399_cost_router055_after_pareto_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V399_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v399_cost_router055_after_pareto_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v399 M100 because M20 gate failed $(date -Is)"
fi
