...
CUDA_VISIBLE_DEVICES=${GPU_V59:-0} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v59_selective_counterfactual_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v59_selective_counterfactual_20260709.json \
  > logs/riskkv_v59_selective_counterfactual_m20_20260709.log 2>&1 < /dev/null &
echo "V59_SELECTIVE_COUNTERFACTUAL_M20_PID=$!"

CUDA_VISIBLE_DEVICES=${GPU_V60:-1} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v60_selective_counterfactual_qasper_full_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v60_selective_counterfactual_qasper_full_20260709.json \
  > logs/riskkv_v60_selective_counterfactual_qasper_full_m20_20260709.log 2>&1 < /dev/null &
echo "V60_SELECTIVE_COUNTERFACTUAL_QASPER_FULL_M20_PID=$!"
