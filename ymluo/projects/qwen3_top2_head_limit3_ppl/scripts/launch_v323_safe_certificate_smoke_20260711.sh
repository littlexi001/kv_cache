...
STAMP="${STAMP:-20260711_v323_safe_certificate_smoke}"
...
launch_one() {
  ...
  nohup env \
    ...
    LABEL="$label" \
    STAMP="$STAMP" \
    POLICY="configs/riskkv_task_policy_v323_safe_certificate_smoke_20260711.json" \
    TASKS="$task" \
...
for task in narrativeqa musique; do
  launch_one "v323_safe_certificate_${task}" "$task"
done

echo "Submitted v323 safe-certificate smoke jobs."
