...
RUN_ROOT="${RUN_ROOT:-${ROOT}/results/20260715_pca_int8_budget_probe_32k_w0}"

mkdir -p "${RUN_ROOT}"
topics=(sports medicine sports medicine sports medicine sports medicine)
fractions=(0.005 0.005 0.01 0.01 0.02 0.02 0.02 0.02)
dimensions=(32 32 32 32 32 32 48 48)

for gpu in {0..7}; do
  topic="${topics[$gpu]}"
  fraction="${fractions[$gpu]}"
  dimension="${dimensions[$gpu]}"
  name="${topic}_w0_m${dimension}_f${fraction}"
  nohup env \
    ...
    "${PYTHON_BIN}" "${ROOT}/src/run_adaptive_mass_budget_ppl_20260715.py" \
      ...
      --qabs_projection_dim "${dimension}" \
      --prefill_chunk_tokens 2048 \
      >"${RUN_ROOT}/${name}.log" 2>&1 </dev/null &
  echo "launched pid=$! gpu=${gpu} topic=${topic} m=${dimension} fraction=${fraction}"
done
