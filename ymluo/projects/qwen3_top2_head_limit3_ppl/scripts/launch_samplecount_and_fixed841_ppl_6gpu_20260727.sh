...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/results/20260727_samplecount_and_fixed841_ppl}"
...
for gpu in {0..5}; do
  topic="${topics[$gpu]}"
  (
    run_one "$gpu" pca_hierarchical_841 "$topic" 0.06 \
      "fixed841_cuda_${topic}"
    run_one "$gpu" pca_hierarchical_841sample20n512 "$topic" 0.12 \
      "sample512_${topic}"
    run_one "$gpu" pca_hierarchical_841sample20n1024 "$topic" 0.12 \
      "sample1024_${topic}"
  ) &
done
