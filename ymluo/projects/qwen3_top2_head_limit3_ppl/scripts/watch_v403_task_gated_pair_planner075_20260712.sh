...
STAMP="${STAMP:-20260712_task_gated_pair075_v403}"
LABEL="${LABEL:-v403_task_gated_pair_planner075}"
POLICY="${POLICY:-configs/riskkv_task_policy_v403_task_gated_pair_planner075_20260712.json}"
...
echo "TRAIN v403 task-gated pair planner <=7.5% $(date -Is)"
"$PY" scripts/train_task_gated_pair_planner_v402_20260712.py \
  --base-action policy_v382 \
  --quality-ratio 0.95 \
  --quality-margin 0.05 \
  --kv-limit 0.075 \
  --cal-tol 0.005 \
  ...
  --output-dir outputs/riskkv_v19_task_gated_pair_planner_v403_075_20260712 \
  --config-out "$POLICY"
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v403_task_gated_pair_planner075_20260712_task_gated_pair075_v403_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v403_task_gated_pair_planner075_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V403_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v403_task_gated_pair_planner075_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v403 M100 because M20 gate failed $(date -Is)"
fi
