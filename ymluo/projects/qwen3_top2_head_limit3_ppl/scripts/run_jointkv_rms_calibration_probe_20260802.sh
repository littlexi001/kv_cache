...
OUTPUT="$ROOT/results/20260802_jointkv_rms_calibration"
...
run_one() {
  local gpu=$1 name=$2 model=$3 cache=$4 text=$5 history=$6 eval_tokens=$7 coupling=$8
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    --score_error_calibration_samples -1 \
    ...
    --codebook_cache "$cache" --output "$OUTPUT/$name.json" \
    >"$OUTPUT/$name.log" 2>&1
}

run_one 0 q06_author4k_ind "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 independent &
run_one 1 q06_author4k_loo "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 previous_layer_quantile &
run_one 2 q06_biomed8k_ind "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 independent &
run_one 3 q06_biomed8k_loo "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 previous_layer_quantile &
run_one 4 q8_author4k_ind "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 independent &
run_one 5 q8_author4k_loo "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 previous_layer_quantile &
run_one 6 q8_author8k_loo "$MODEL8" "$CACHE8" qksieve_author_text.txt 8192 8 previous_layer_quantile &
run_one 7 q8_author1k_loo "$MODEL8" "$CACHE8" qksieve_author_text.txt 1024 16 previous_layer_quantile &
wait
