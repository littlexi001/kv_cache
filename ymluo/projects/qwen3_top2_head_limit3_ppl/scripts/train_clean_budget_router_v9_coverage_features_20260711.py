...
def train(task_encoding: str, class_weight: str, confidence: str, max_depth: int, min_leaf: int) -> None:
    ...
    output_dir = (
        f"outputs/riskkv_v19_learned_budget_router_v9_coverage_features_"
        f"covered320_allfeatures_{task_encoding}_{class_weight}_d{max_depth}_l{min_leaf}_conf{conf_tag}_20260711"
    )
    command = [
        PYTHON,
...
def main() -> None:
    for task_encoding in ["both", "family"]:
        for class_weight in ["none", "balanced"]:
            for max_depth, min_leaf in [(4, 4), (5, 3), (6, 3)]:
                for confidence in ["0.0", "0.25", "0.35", "0.45", "0.55"]:
