...
OUTPUT="${OUTPUT:-${ROOT}/results/20260803_qksieve_mass_ladder_4gpu_v1}"
...
run_case() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -u \
    ...
    --device cuda \
    --fixed_top_k 1280 \
    --coverage_targets 0.85,0.90,0.95 \
    --mass_ladder_samples 256,1024 \
    --mass_ladder_growth 2 \
    --minimum_top_k 1280 \
    --maximum_top_k 0 \
    --key_rate_budget 15 \
    --value_rank 16 \
...
failed=0
for pid in "${pid0}" "${pid1}" "${pid2}" "${pid5}"; do
  if ! wait "${pid}"; then
    failed=1
  fi
done
