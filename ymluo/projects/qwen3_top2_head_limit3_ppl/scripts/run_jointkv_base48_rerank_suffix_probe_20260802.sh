...
OUTPUT="$ROOT/results/20260802_jointkv_base48_rerank_suffix_probe"
CACHE="$OUTPUT/qwen06b_train5_cal1024_jointkv_base48_res48_codebooks.pt"
...
COMMON=(
  ...
  --binary_bits 48 --projection_iterations 6
  ...
  --adaptive_error_tolerance 0.05 --adaptive_action_mode joint_cost_rerank
  ...
  --one_shot_feedback_source suffix_token
...
if [[ ! -s "$CACHE" ]]; then
  CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    --history_tokens 4096 --eval_tokens 2 \
    --output "$OUTPUT/cache_fit_smoke.json" \
    >"$OUTPUT/cache_fit_smoke.log" 2>&1
fi

run_one() {
  local gpu=$1 name=$2 text=$3 history=$4
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "$RUNNER" "${COMMON[@]}" \
    --test_texts "$DATA/$text" --history_tokens "$history" --eval_tokens 16 \
    --output "$OUTPUT/$name.json" >"$OUTPUT/$name.log" 2>&1
}

run_one 0 q06_author_4k_e16_base48 qksieve_author_text.txt 4096 &
run_one 1 q06_biomed_8k_e16_base48 biomed_long_range_facts_hard_compact.txt 8192 &
run_one 2 q06_compiler_8k_e16_base48 compiler_long_range_facts_hard_compact.txt 8192 &
run_one 3 q06_geology_8k_e16_base48 geology_long_range_facts_hard_compact.txt 8192 &
wait
