...
LOG=${LOG:-logs/watch_and_launch_v59_v60_after_v57_v58_20260709.log}
...
already_running() {
  pgrep -af "riskkv_v[56][90]_selective_counterfactual.*m20_20260709" >/dev/null 2>&1
}
...

{
  ...
  for ((i = 1; i <= MAX_WAIT_LOOPS; i++)); do
    if already_done; then
      echo "[$(date)] v59/v60 selective-counterfactual outputs already complete"
      exit 0
    fi
    if already_running; then
      echo "[$(date)] v59/v60 selective-counterfactual already running"
      exit 0
    fi
    if ! prereq_done; then
    ...
    if (( ${#gpus[@]} >= 2 )); then
      GPU_V59=${gpus[0]} GPU_V60=${gpus[1]} bash scripts/run_riskkv_v59_v60_selective_counterfactual_m20_20260709.sh
      echo "[$(date)] launched v59/v60 on GPUs ${gpus[0]} ${gpus[1]}"
      exit 0
    fi
    sleep "$INTERVAL"
  done
