...
GPUS="${GPUS:-1,0,7,6,4,2,3,5}"
RUNNER="${RUNNER:-scripts/run_riskkv_task_policy_v19_one_20260709.sh}"
STAMP="${STAMP:-20260712_completed_m100_055_v401}"
LABEL="${LABEL:-v401_cost_router055_completed_m100}"
POLICY="${POLICY:-configs/riskkv_task_policy_v401_cost_router055_completed_m100_20260712.json}"
...
echo "TRAIN v401 cost-aware router kv<=5.5% from currently completed M100 candidates $(date -Is)"
"$PY" scripts/train_cost_aware_router_v397_after_pareto_20260712.py \
  --output-dir outputs/riskkv_v19_cost_router_v401_completed_m100_055_20260712 \
  --config-out "$POLICY" \
  --kv-limit 0.055 \
  --cal-min-gain -0.001 \
  --test-min-gain 0.0
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v401_cost_router055_completed_m100_20260712_completed_m100_055_v401_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v401_cost_router055_completed_m100_m100_gate_20260712.flag")
...
passed = score / full_score >= 0.95 and 0.01 <= kv <= 0.065 and speed >= 2.5
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V401_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v401_cost_router055_completed_m100_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v401 M100 because M20 gate failed $(date -Is)"
fi
