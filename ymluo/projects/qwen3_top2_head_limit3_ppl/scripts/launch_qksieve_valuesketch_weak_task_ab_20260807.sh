...
RUN_ROOT="${RUN_ROOT:-${ROOT}/results/longbench_llama8b_weak3_novaluesketch_m5_20260807}"
GPUS="${GPUS:-0,1,2}"
NUM_SHARDS="${NUM_SHARDS:-3}"
QK_WORKERS="${QK_WORKERS:-12}"
...
for required in "${MODEL}/config.json" "${DATA_DIR}/manifest.json"; do
  if [[ ! -f "${required}" ]]; then
...
{
  echo "purpose=debug-only ValueSketch quality isolation"
  echo "model=${MODEL}"
  echo "tasks=${TASKS}"
  echo "max_samples_per_task=5"
  echo "methods=${METHOD}"
  echo "QKSIEVE_DEBUG_DISABLE_VALUE_SKETCH=1"
