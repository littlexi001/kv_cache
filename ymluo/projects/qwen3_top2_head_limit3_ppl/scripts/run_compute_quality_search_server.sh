...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_DIR}/outputs/compute_quality_search_server}"

PREFILL_LENGTHS="${PREFILL_LENGTHS:-10000 20000 40000 60000}"
EVAL_TOKENS="${EVAL_TOKENS:-1000}"
CHUNK_SIZE="${CHUNK_SIZE:-8}"
EVAL_CHUNK_SIZE="${EVAL_CHUNK_SIZE:-1}"
MODES="${MODES:-baseline,top2,qabs8cand3reuse,qabs8cand5reuse,qabs8cand7reuse,qabs16cand3reuse,qabs16cand7reuse,qabs16cand10reuse,qabs32cand10reuse}"
...
for prefill_tokens in ${PREFILL_LENGTHS}; do
  ...
  echo "=== compute-quality search prefill=${prefill_tokens} eval=${EVAL_TOKENS} prefill_chunk=${CHUNK_SIZE} eval_chunk=${EVAL_CHUNK_SIZE} ==="
