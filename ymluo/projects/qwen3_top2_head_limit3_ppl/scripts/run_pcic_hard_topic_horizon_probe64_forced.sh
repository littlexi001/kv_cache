...
OUT=outputs/server_pcic_hardtopic_b4_horizonprobe64_forced_seed64_slack0_margin0_eager
if [[ ! -f "$OUT/pcic_r_blockwise_results.csv" ]]; then
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" "$PY" src/run_pcic_rescue_blockwise_local.py \
    ...
    > outputs/logs/server_pcic_hardtopic_b4_horizonprobe64_forced_seed64_slack0_margin0_eager.log 2>&1
fi
...
path = pathlib.Path("outputs/server_pcic_hardtopic_b4_horizonprobe64_forced_seed64_slack0_margin0_eager/pcic_r_blockwise_results.csv")
