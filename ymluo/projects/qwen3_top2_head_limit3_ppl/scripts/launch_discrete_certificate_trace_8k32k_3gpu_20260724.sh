...
RUN_ROOT=$ROOT/results/20260724_discrete_certificate_trace_8k32k_3gpu
...
for spec in "0:8192" "1:16000" "2:32000"; do
  ...
  (
    ...
    "$PYTHON" src/analyze_discrete_certificate_trace_20260724.py \
      --trace_path "$RUN_ROOT/length${length}.pt" \
      --output_path "$RUN_ROOT/length${length}_certificate.json" \
      >> "$RUN_ROOT/logs/length${length}.log" 2>&1
  ) &
  pids+=("$!")
done
