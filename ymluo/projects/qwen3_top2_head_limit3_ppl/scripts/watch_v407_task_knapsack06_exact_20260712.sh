...
STAMP="${STAMP:-20260712_task_knapsack06_v407}"
LABEL="${LABEL:-v407_task_knapsack06_exact}"
POLICY="${POLICY:-configs/riskkv_task_policy_v407_task_knapsack06_exact_20260712.json}"
...
echo "BUILD v407 exact task knapsack 6% $(date -Is)"
"$PY" scripts/build_task_knapsack_policy_v407_20260712.py \
  --kv-limit 0.06 \
  --config-out "$POLICY" \
  --summary-out outputs/riskkv_v19_v407_task_knapsack06_exact_20260712/summary.json
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v407_task_knapsack06_exact_20260712_task_knapsack06_v407_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v407_task_knapsack06_exact_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V407_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v407_task_knapsack06_exact_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v407 M100 because M20 gate failed $(date -Is)"
fi
