...
RUN_ROOT=$ROOT/results/20260729_qksieve_frequency_tiered_ppl_heldout32k
...
run_case() {
  local tag=$1
  local mode=$2
  if [[ -f "$RUN_ROOT/$tag/case_summary.json" ]]; then
    return
  fi
...
run_case \
  freqtier \
  pca_hierarchical_autoqmsetotal15z_qkmetric_freqtier12hot4_packed_fulltopk
