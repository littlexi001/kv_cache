...
RUN_ROOT=$ROOT/results/20260724_cacheauto_validation_8k16k_2gpu
AUTO=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_qkvsplitauto_prefillindex
CACHE_AUTO=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_qkvsplitauto_cacheauto_prefillindex
INPLACE=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_qkvsplitauto_inplacecache_prefillindex
...
run_length() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" -u \
    src/run_sample_calibrated_longbench_20260717.py \
      ...
      --output_dir "$RUN_ROOT/${length}k" \
      --tasks gov_report \
      --methods "$AUTO,$CACHE_AUTO,$INPLACE" \
      ...
      > "$RUN_ROOT/logs/${length}k.log" 2>&1
}
