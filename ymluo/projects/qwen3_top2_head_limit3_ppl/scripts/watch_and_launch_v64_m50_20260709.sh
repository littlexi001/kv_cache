...
LOG=${LOG:-logs/watch_and_launch_v64_m50_20260709.log}
LAUNCH_LOCK=${LAUNCH_LOCK:-logs/riskkv_gpu_launch.lock}
MAX_WAIT_LOOPS=${MAX_WAIT_LOOPS:-720}
NAME=${NAME:-riskkv_v64_benefit_conformal_counterfactual_qasper_full_m50_20260709}
POLICY=${POLICY:-configs/riskkv_task_policy_v64_benefit_conformal_counterfactual_qasper_full_20260709.json}
...
{
  echo "[$(date)] v64 m50 watcher started"
  for ((i = 1; i <= MAX_WAIT_LOOPS; i++)); do
    ...
    if (( ${#gpus[@]} >= 1 )); then
      ...
      GPU="${gpus[0]}" MAX_SAMPLES=50 NAME="$NAME" POLICY="$POLICY" \
