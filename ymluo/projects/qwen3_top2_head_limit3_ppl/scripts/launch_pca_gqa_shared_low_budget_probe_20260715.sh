...
RUN_ROOT="${RUN_ROOT:-${ROOT}/results/20260715_pca_gqa_shared_low_budget_probe_32k_w0}"
mkdir -p "${RUN_ROOT}"

topics=(sports medicine sports medicine sports medicine sports medicine)
modes=(shared_mean shared_mean shared_mean shared_mean shared_zmax shared_zmax shared_zmax shared_zmax)
fractions=(0.005 0.005 0.01 0.01 0.005 0.005 0.01 0.01)
for gpu in {0..7}; do
  topic="${topics[$gpu]}"
  mode="${modes[$gpu]}"
  fraction="${fractions[$gpu]}"
  name="${topic}_w0_${mode}_f${fraction}"
  nohup env \
    ...
    "${PYTHON_BIN}" "${ROOT}/src/run_adaptive_mass_budget_ppl_20260715.py" \
      ...
      --qabs_gqa_candidate_mode "${mode}" \
      --prefill_chunk_tokens 2048 \
      >"${RUN_ROOT}/${name}.log" 2>&1 </dev/null &
  echo "launched pid=$! gpu=${gpu} topic=${topic} mode=${mode} fraction=${fraction}"
done
