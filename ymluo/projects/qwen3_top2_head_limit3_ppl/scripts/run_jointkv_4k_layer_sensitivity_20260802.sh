...
OUTPUT="$ROOT/results/20260802_jointkv_all_layer_ppl/remote_4k_layer_sensitivity"
...
COMMON=(
  ...
  --fraction 0.20 --recent_tokens 128 --sink_tokens 4
...
run_one() {
  local gpu=$1 name=$2 layers=$3
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "${COMMON[@]}" --sparse_layers "$layers" \
    --output "$OUTPUT/$name.json" >"$OUTPUT/$name.log" 2>&1
}

run_one 0 layers_00_06 0-6 &
run_one 1 layers_07_13 7-13 &
run_one 2 layers_14_20 14-20 &
run_one 3 layers_21_27 21-27 &
wait
