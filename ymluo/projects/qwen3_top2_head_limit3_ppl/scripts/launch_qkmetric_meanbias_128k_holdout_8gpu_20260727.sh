#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${PYTHON:-/home/fdong/miniconda3/envs/moe/bin/python}"
MODEL="${MODEL:-/home/fdong/models/Qwen3-4B-Instruct}"
OUTPUT="${OUTPUT:-$ROOT/results/20260727_qkmetric_meanbias_128k_holdout}"
SCORE_MODE="${SCORE_MODE:-pca_hierarchical_autoqmsetotal15z_qkmetric_qscale_meanbias_packed_direct}"
QK_SHRINKAGE="${QK_SHRINKAGE:-0.75}"

export PATH="/home/fdong/miniconda3/envs/moe/bin:/usr/local/cuda/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export TORCH_CUDA_ARCH_LIST=8.6
mkdir -p "$OUTPUT/logs"

run_case() {
  local devices="$1"
  local topic="$2"
  local window="$3"
  local output="$OUTPUT/${topic}_w${window}"
  local log="$OUTPUT/logs/${topic}_w${window}.log"

  if [[ -s "$output/case_summary.json" ]]; then
    echo "SKIP ${topic} window ${window}"
    return
  fi

  echo "START ${topic} window ${window} on GPUs ${devices}"
  CUDA_VISIBLE_DEVICES="$devices" "$PYTHON" -u \
    "$ROOT/src/run_direct_countcap_denseprompt_ppl_20260725.py" \
    --model_name_or_path "$MODEL" \
    --output_dir "$output" \
    --topics "$topic" \
    --window_indices "$window" \
    --methods direct_countcap \
    --history_tokens 128000 \
    --eval_tokens 256 \
    --window_stride_tokens 128512 \
    --direct_fraction 0.06 \
    --direct_min_tokens 256 \
    --direct_max_tokens 1280 \
    --sample_count 256 \
    --candidate_overfetch 1.0 \
    --direct_score_mode "$SCORE_MODE" \
    --qk_metric_query_shrinkage "$QK_SHRINKAGE" \
    --prefill_chunk_tokens 1024 \
    --cache_mode preallocated \
    --dataset_cache_dir /home/fdong/ymluo/datasets/sklearn \
    --dtype float16 \
    --device cuda \
    --device_map balanced \
    >"$log" 2>&1
  echo "DONE ${topic} window ${window}"
}

run_case "0,1" mixed_a 2 &
pid0=$!
run_case "2,3" mixed_a 3 &
pid1=$!
run_case "4,5" mixed_b 2 &
pid2=$!
run_case "6,7" mixed_b 3 &
pid3=$!

wait "$pid0"
wait "$pid1"
wait "$pid2"
wait "$pid3"
touch "$OUTPUT/ALL_COMPLETE"
echo "ALL_COMPLETE"
