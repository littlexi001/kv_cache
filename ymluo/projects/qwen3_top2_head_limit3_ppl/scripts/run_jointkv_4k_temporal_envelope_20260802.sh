...
OUTPUT="$ROOT/results/20260802_jointkv_all_layer_ppl/remote_4k_temporal_envelope"
...
COMMON=(
  ...
  --adaptive_budget_coupling previous_layer_max
...
run_one() {
  local gpu=$1 name=$2 tolerance=$3
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "${COMMON[@]}" --adaptive_error_tolerance "$tolerance" \
    --output "$OUTPUT/$name.json" >"$OUTPUT/$name.log" 2>&1
}

run_one 0 tau0025 0.025 &
run_one 1 tau0035 0.035 &
run_one 2 tau0050 0.050 &
wait
