...
RUN_ROOT=$ROOT/results/20260727_qk_progressive_refinement_32k
...
while [[ ! -e "$PREREQUISITE/ALL_COMPLETE" ]]; do
  if ! pgrep -f '^bash scripts/launch_qk_variable_physical_32k_paired_8gpu_20260727.sh$' >/dev/null; then
    echo "32K physical prerequisite exited without ALL_COMPLETE" >&2
    exit 1
  fi
  sleep 60
done
...
for topic in "${topics[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=7 "$PYTHON" -u \
    src/analyze_qk_progressive_refinement_20260727.py \
    --trace_path "$trace" \
    --output_dir "$output" \
    ...
    --full_rate_budget 15 \
    --base_rate_budgets 5,7,9,11,13 \
    --selected_fractions 0.01,0.04 \
    --alphas 0.05,0.02,0.01,0.005 \
    --current_query_samples 256 \
    >"$LOG_ROOT/${topic}.log" 2>&1
done

"$PYTHON" src/summarize_qk_progressive_refinement_20260727.py \
  --input_root "$RUN_ROOT" \
