...
RUN_ROOT=$ROOT/results/20260724_temporal_gqa_massgate_shadow_8k32k_3gpu
BASE=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_prefillindex
GQA94=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_gqamassgate94_prefillindex
GQA95=countcap_fullprompt_keypca_direct_qkvfused_qprojscan_gqamassgate95_prefillindex
...
run_length() {
  local gpu=$1
  local length=$2
  local out="$RUN_ROOT/length${length}"
  mkdir -p "$out"
