...
PREFIX=20260716_hierarchical_ruler_4k32k_m10_shard
EXAMPLES=$ROOT/data/ruler_generated/llama31_8b_4k32k_m10_seed42.jsonl
...
for shard in $(seq 0 7); do
  ...
  CUDA_VISIBLE_DEVICES="$shard" taskset -c "$cpus" nohup "$PYTHON" \
    ...
    --device_map auto \
    >> "$log" 2>&1 &
  echo "ruler-stage1 shard=$shard pid=$! log=$log"
done
