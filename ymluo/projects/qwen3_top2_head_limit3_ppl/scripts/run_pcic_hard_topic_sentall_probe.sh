...
OUT=outputs/server_pcic_hardtopic_b4_sentall_s8_seed64_slack03_margin01_eager
if [[ ! -f "$OUT/pcic_r_blockwise_results.csv" ]]; then
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" "$PY" src/run_pcic_rescue_blockwise_local.py \
    ...
    > outputs/logs/server_pcic_hardtopic_b4_sentall_s8_seed64_slack03_margin01_eager.log 2>&1
fi
...
path = pathlib.Path("outputs/server_pcic_hardtopic_b4_sentall_s8_seed64_slack03_margin01_eager/pcic_r_blockwise_results.csv")
