...
OUTPUT="${OUTPUT:-${ROOT}/results/20260803_qksieve_rate15_safety2_control_2gpu_v1}"
DATASET_CACHE_DIR="${DATASET_CACHE_DIR:-/home/fdong/ymluo/datasets/sklearn}"
SCORE_MODE="pca_hierarchical_autoqmsetotal15z_qkmetric_valuesketch16i4shared_wometric_residualrisk4_prefixrss25e4_safety2_packed_fulltopk_oas"
...
run_case() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -u \
    ...
    --topics "${topic}" --window_indices 0 \
    --methods full_attention,direct_countcap \
    --history_tokens "${history_tokens}" --eval_tokens 8 \
...
run_case 0 religion4k_rate15_safety2 "${LLAMA_MODEL}" religion 4096 & pid0=$!
run_case 1 sports32k_rate15_safety2 "${QWEN_MODEL}" sports 32000 & pid1=$!
