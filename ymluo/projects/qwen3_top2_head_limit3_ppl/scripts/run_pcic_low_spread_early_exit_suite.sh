...
run_case() {
  ...
  echo "[start] $name anchors=$anchors margin=$ANCHOR_ACCEPT_MARGIN low_spread=$LOW_SPREAD"
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" "$PY" src/run_pcic_rescue_blockwise_local.py \
    ...
    --sentinel_cascade_accept_low_spread "$LOW_SPREAD" \
...
echo "[low-spread-early-exit] hard=${HARD_ANCHORS} war=${WAR_ANCHORS} monte=${MONTE_ANCHORS} needle=${NEEDLE_ANCHORS} low_spread=${LOW_SPREAD}"

run_case server_pcic_hardtopic_b4_horizongate_lowspread_eval128_seed64_eager \
  "$HARD" 4 128 128 64 "$HARD_COMBOS" "$HARD_ANCHORS"
run_case server_pcic_war_b2_horizongate_lowspread_seed64_eager \
  data/war_and_peace_pg2600.txt 2 64 64 32 "$WAR_COMBOS" "$WAR_ANCHORS"
run_case server_pcic_monte_b2_horizongate_lowspread_seed64_eager \
  data/count_monte_cristo_pg1184.txt 2 64 64 32 "$MONTE_COMBOS" "$MONTE_ANCHORS"
run_case server_pcic_needle_b4_horizongate_lowspread_eval128_seed64_eager \
...
cases = [
    ...
    ("hard_lowspread", pathlib.Path("outputs/server_pcic_hardtopic_b4_horizongate_lowspread_eval128_seed64_eager/pcic_r_blockwise_results.csv")),
    ("war_cond", pathlib.Path("outputs/server_pcic_war_b2_horizongate_condautoanchor_seed64_eager/pcic_r_blockwise_results.csv")),
    ("war_lowspread", pathlib.Path("outputs/server_pcic_war_b2_horizongate_lowspread_seed64_eager/pcic_r_blockwise_results.csv")),
    ...
    ("monte_lowspread", pathlib.Path("outputs/server_pcic_monte_b2_horizongate_lowspread_seed64_eager/pcic_r_blockwise_results.csv")),
    ...
    ("needle_lowspread", pathlib.Path("outputs/server_pcic_needle_b4_horizongate_lowspread_eval128_seed64_eager/pcic_r_blockwise_results.csv")),
]

print("| run | avg_delta_ppl | method/base | gate_s | extended | early | low_spread_early | anchors | combos |")
print("|---|---:|---:|---:|---:|---:|---:|---|---|")
for name, path in cases:
    ...
    print(
        ...
        f"{sum(int(rule.get('sentinel_cascade_accepted_by_low_spread', 0) or 0) for rule in rules)} | "
