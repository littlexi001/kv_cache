...
OUTPUT="$ROOT/results/20260802_jointkv_warp_local_quality_probe"
...
COMMON=(
  ...
  --fraction 0.06 --recent_tokens 128 --sink_tokens 4 --sparse_layers all
  ...
  --residual_binary_candidate_fraction 0.20
...
run_one() {
  ...
  local text
  case "$topic" in
    biomed) text="$DATA/biomed_long_range_facts_hard_compact.txt" ;;
    compiler) text="$DATA/compiler_long_range_facts_hard_compact.txt" ;;
    *) echo "unknown topic: $topic" >&2; exit 2 ;;
  esac
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "${COMMON[@]}" --test_texts "$text" \
    --candidate_selection "$selector" \
    --warp_local_base_keep 8 --warp_local_shortlist_multiplier 2.0 \
    --output "$OUTPUT/${topic}_${selector}.json" \
    >"$OUTPUT/${topic}_${selector}.log" 2>&1
}

run_one 0 biomed global &
run_one 1 biomed warp_local &
run_one 2 compiler global &
run_one 3 compiler warp_local &
wait
