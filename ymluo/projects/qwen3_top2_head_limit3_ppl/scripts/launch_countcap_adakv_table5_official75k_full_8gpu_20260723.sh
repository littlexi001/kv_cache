...
"$PYTHON" -c "from rouge import Rouge; assert Rouge"
while [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits | sed '/^[[:space:]]*$/d')" ]]; do
  log "queued: waiting for all GPUs to become idle"
  sleep 30
done
...
log "starting official AdaKV Table-5 protocol: 7.5K prompt, official ROUGE/stops"
