...
STAMP="${STAMP:-20260709_task_bridge_v18_qm_m20}"
...
GPU="$(choose_gpu)"
OUT="outputs/riskkv_v18_task_bridge_qm_m20_${STAMP}_b512_p128"
LOG="$LOG_ROOT/riskkv_v18_task_bridge_qm_m20_${STAMP}_b512_p128.log"
mkdir -p "$OUT"

echo "START v18_task_bridge_qm_m20 gpu=$GPU samples=$SAMPLES tasks=$TASKS $(date -Is)"
CUDA_VISIBLE_DEVICES="$GPU" python src/run_controlled_public_kv_benchmark_v1.py \
  ...
  --log_every 20 \
  > "$LOG" 2>&1
echo "DONE v18_task_bridge_qm_m20 $(date -Is)"
cat "$OUT/summary.csv"
