...
RUN_ROOT="${RUN_ROOT:-${ROOT}/results/20260715_pca_gqa_shared_probe_32k_w0_f0.02}"
mkdir -p "${RUN_ROOT}"

topics=(sports medicine sports medicine sports medicine)
modes=(shared_max shared_max shared_zmax shared_zmax shared_mean shared_mean)
for gpu in {0..5}; do
  topic="${topics[$gpu]}"
  mode="${modes[$gpu]}"
  name="${topic}_w0_${mode}_f0.02"
  nohup env \
  ...
  echo "launched pid=$! gpu=${gpu} topic=${topic} mode=${mode}"
done
