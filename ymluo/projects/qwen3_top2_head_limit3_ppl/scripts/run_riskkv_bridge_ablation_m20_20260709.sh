...
STAMP="${STAMP:-20260709_bridge_ablation_m20}"
...
run_one() {
  ...
  mkdir -p "$out"
  echo "START label=$label scorer=$scorer gpu=$gpu samples=$SAMPLES $(date -Is)"
  CUDA_VISIBLE_DEVICES="$gpu" python src/run_controlled_public_kv_benchmark_v1.py \
    ...
    --log_every 20 \
    > "$log" 2>&1
  echo "DONE label=$label scorer=$scorer $(date -Is)"
  cat "$out/summary.csv"
}

run_one "v13_multiscale_flow_m20" "hybrid_late_mmr_multiscale_flow"
run_one "v16_multiscale_bridge_m20" "hybrid_late_mmr_multiscale_bridge_flow"
