...
def budget_candidate_args() -> list[str]:
    ...
    for budget in BUDGETS:
        ...
        args.extend(["--candidate", f"budget_b{budget}={relative}"])
    return args
...
def train(task_encoding: str, confidence: str, max_depth: int, min_samples_leaf: int) -> None:
    ...
    output_dir = (
        f"outputs/riskkv_v19_learned_budget_router_v4_clean_budget_"
        f"preselection_{task_encoding}_conf{conf_tag}_20260711"
    )
    command = [
        PYTHON,
...
def main() -> None:
    for confidence in ["0.0", "0.35", "0.50", "0.65"]:
