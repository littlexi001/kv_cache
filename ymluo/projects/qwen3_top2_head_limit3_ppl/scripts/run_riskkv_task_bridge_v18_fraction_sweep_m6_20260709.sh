...
STAMP="${STAMP:-20260709_task_bridge_v18_fraction_sweep_m6}"
...
run_one() {
  local frac="$1"
  local label="fast_v18_qm_frac_${frac/./}"
  local gpu
  gpu="$(choose_gpu)"
  local out="outputs/riskkv_${label}_${STAMP}_b512_p128"
  local log="$LOG_ROOT/riskkv_${label}_${STAMP}_b512_p128.log"
  mkdir -p "$out"
  echo "START label=$label bridge_fraction=$frac gpu=$gpu samples=$SAMPLES tasks=$TASKS $(date -Is)"
  CUDA_VISIBLE_DEVICES="$gpu" python src/run_controlled_public_kv_benchmark_v1.py \
    ...
    --ours_bridge_budget_fraction "$frac" \
    ...
    --log_every 12 \
    > "$log" 2>&1
  echo "DONE label=$label bridge_fraction=$frac $(date -Is)"
  cat "$out/summary.csv"
}
