...
PARENT_RUN=$ROOT/results/20260726_qwen25_7b_qk_matrix_spectrum_32k
QWEN25_RUN=$ROOT/results/20260726_countcap_qwen25_7b_longbench_m100_prompt7500
LOG=$PARENT_RUN/logs/three_model_report.log

cd "$ROOT"
while [[ ! -f "$PARENT_RUN/ALL_COMPLETE" ]]; do
  if [[ -f "$PARENT_RUN/FAILED" ]]; then
    printf '%s Qwen2.5 spectrum parent failed\n' "$(date -Is)" >"$LOG"
    exit 1
  fi
  if ! pgrep -f \
    "launch_qwen25_qk_spectrum_after_longbench_2gpu_20260726.sh" \
    >/dev/null; then
    printf '%s spectrum parent stopped without ALL_COMPLETE\n' \
      "$(date -Is)" >"$LOG"
    exit 1
  fi
  sleep 300
