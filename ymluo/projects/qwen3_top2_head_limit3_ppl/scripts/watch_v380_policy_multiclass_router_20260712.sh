...
STAMP="${STAMP:-20260712_policy_multiclass_router_v380}"
LABEL="${LABEL:-v380_policy_multiclass_router}"
POLICY="${POLICY:-configs/riskkv_task_policy_v380_policy_multiclass_router_20260712.json}"
...
echo "TRAIN_V380_START $(date -Is)"
"$PY" scripts/train_policy_multiclass_router_v380_20260712.py
echo "TRAIN_V380_DONE $(date -Is)"
...
full_online = 3.0988
full_score = 0.3658
path = Path("outputs/riskkv_v19_v380_policy_multiclass_router_20260712_policy_multiclass_router_v380_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v380_policy_multiclass_router_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(
    f"V380_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} "
    f"kv={kv:.2%} speed_full={speed:.2f}x passed={passed}"
)
PY

if [[ "$(cat outputs/riskkv_v19_v380_policy_multiclass_router_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v380 M100 because M20 gate failed $(date -Is)"
fi
