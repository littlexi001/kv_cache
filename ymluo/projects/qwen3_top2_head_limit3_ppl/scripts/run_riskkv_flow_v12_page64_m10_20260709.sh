...
STAMP="${STAMP:-20260709_flow_v12_page64_m10}"
...
run_one() {
  ...
  local scorer="$2"
  local budget="$3"
  local out="outputs/riskkv_${label}_${STAMP}_b${budget}_p64"
  local log="$LOG_ROOT/riskkv_${label}_${STAMP}_b${budget}_p64.log"
  mkdir -p "$out"
  echo "START label=$label scorer=$scorer budget=$budget gpu=$GPU samples=$SAMPLES $(date -Is)"
  CUDA_VISIBLE_DEVICES="$GPU" python src/run_controlled_public_kv_benchmark_v1.py \
    ...
    --page_tokens 64 \
    --ours_scorer "$scorer" \
    --ours_flow_neighbor_radius 2 \
    --ours_flow_neighbor_budget_fraction 0.25 \
    --ours_flow_neighbor_min_score 0.12 \
    --ours_flow_score_smooth_weight 0.22 \
    --ours_flow_anchor_boost 0.22 \
    --dtype float16 \
    ...
    --log_every 20 \
    > "$log" 2>&1
  echo "DONE label=$label scorer=$scorer budget=$budget $(date -Is)"
  cat "$out/summary.csv"
}

run_one "v11_hybrid_late_mmr_page64" "hybrid_late_mmr" 512
run_one "v12_flow_page64" "hybrid_late_mmr_flow" 512
run_one "v12_flow_page64_conservative" "hybrid_late_mmr_flow" 1024
