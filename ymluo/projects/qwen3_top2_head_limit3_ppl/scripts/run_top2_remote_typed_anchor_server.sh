...
OUT=/home/fdong/ymluo/projects/qwen3_top2_head_limit3_ppl/outputs/top2_remote_typed_anchor_war_4k_s64_r512_v1
mkdir -p "$OUT"
...
nohup bash -lc "
python -u src/analyze_top2_token_selection_counts.py \
  ...
  --accumulate_selected_attention_mass true

python -u src/summarize_top2_remote_anchor_types.py \
  --input_dir '$OUT' \
  --page_size 64 \
  --top_tokens_per_type 100
