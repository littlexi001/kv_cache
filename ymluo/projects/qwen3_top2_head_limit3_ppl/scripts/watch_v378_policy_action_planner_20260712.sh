...
STAMP="${STAMP:-20260712_policy_action_planner_v378}"
LABEL="${LABEL:-v378_policy_action_planner}"
POLICY="${POLICY:-configs/riskkv_task_policy_v378_policy_action_planner_20260712.json}"
...
echo "TRAIN_V378_START $(date -Is)"
"$PY" scripts/train_policy_action_planner_v378_20260712.py
echo "TRAIN_V378_DONE $(date -Is)"
...
full_online = 3.0988
full_score = 0.3658
path = Path("outputs/riskkv_v19_v378_policy_action_planner_20260712_policy_action_planner_v378_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v378_policy_action_planner_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(
    f"V378_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} "
    f"kv={kv:.2%} speed_full={speed:.2f}x passed={passed}"
)
PY

if [[ "$(cat outputs/riskkv_v19_v378_policy_action_planner_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v378 M100 because M20 gate failed $(date -Is)"
fi
