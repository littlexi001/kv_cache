...
CUDA_VISIBLE_DEVICES=${GPU_V63:-0} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v63_benefit_conformal_counterfactual_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v63_benefit_conformal_counterfactual_20260709.json \
  > logs/riskkv_v63_benefit_conformal_counterfactual_m20_20260709.log 2>&1 < /dev/null &
echo "V63_BENEFIT_CONFORMAL_M20_PID=$!"

CUDA_VISIBLE_DEVICES=${GPU_V64:-1} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v64_benefit_conformal_counterfactual_qasper_full_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v64_benefit_conformal_counterfactual_qasper_full_20260709.json \
  > logs/riskkv_v64_benefit_conformal_counterfactual_qasper_full_m20_20260709.log 2>&1 < /dev/null &
echo "V64_BENEFIT_CONFORMAL_QASPER_FULL_M20_PID=$!"
