...
STAMP="${STAMP:-20260712_task_gated_pair_v402}"
LABEL="${LABEL:-v402_task_gated_pair_planner}"
POLICY="${POLICY:-configs/riskkv_task_policy_v402_task_gated_pair_planner_20260712.json}"
...
echo "TRAIN v402 task-gated pair planner $(date -Is)"
"$PY" scripts/train_task_gated_pair_planner_v402_20260712.py \
  ...
  --output-dir outputs/riskkv_v19_task_gated_pair_planner_v402_20260712 \
  --config-out "$POLICY"
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v402_task_gated_pair_planner_20260712_task_gated_pair_v402_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v402_task_gated_pair_planner_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V402_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v402_task_gated_pair_planner_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v402 M100 because M20 gate failed $(date -Is)"
fi
