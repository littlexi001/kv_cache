...
STAMP="${STAMP:-20260712_pair_after_pareto075_v405}"
LABEL="${LABEL:-v405_pair_planner_after_pareto075}"
POLICY="${POLICY:-configs/riskkv_task_policy_v405_pair_planner_after_pareto075_20260712.json}"
...
echo "TRAIN v405 after-pareto pair planner <=7.5% $(date -Is)"
"$PY" scripts/train_task_gated_pair_planner_v402_20260712.py \
  --base-action policy_v396 \
  --quality-ratio 0.95 \
  --quality-margin 0.05 \
  --kv-limit 0.075 \
  --cal-tol 0.005 \
  --test-tol 0.005 \
  --max-kv-multiplier 2.5 \
  --output-dir outputs/riskkv_v19_pair_planner_after_pareto075_v405_20260712 \
  --config-out "$POLICY"
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v405_pair_planner_after_pareto075_20260712_pair_after_pareto075_v405_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v405_pair_planner_after_pareto075_m100_gate_20260712.flag")
...
passed = score / full_score >= 0.95 and 0.01 <= kv <= 0.080 and speed >= 2.5
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V405_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v405_pair_planner_after_pareto075_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v405 M100 because M20 gate failed $(date -Is)"
fi
