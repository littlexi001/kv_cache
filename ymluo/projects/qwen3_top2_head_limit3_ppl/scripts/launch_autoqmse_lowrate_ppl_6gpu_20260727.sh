...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/results/20260727_autoqmse_lowrate_ppl}"
...
for gpu in {0..5}; do
  topic="${topics[$gpu]}"
  (
    run_one "$gpu" pca_hierarchical_autoqmse10z "$topic" \
      "autoqmse10z_${topic}"
    run_one "$gpu" pca_hierarchical_autoqmse12z "$topic" \
      "autoqmse12z_${topic}"
  ) &
done
