...
OUTPUT="$ROOT/results/20260802_jointkv_output_feedback_probe"
...
COMMON=(
  ...
  --store_budget_records
)

run_one() {
  local gpu=$1 name=$2 model=$3 cache=$4 text=$5 history=$6 eval_tokens=$7 feedback=$8
  local extra=()
  if [[ "$feedback" == "feedback" ]]; then
    extra+=(--one_shot_output_error_feedback)
  fi
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    "${extra[@]}" --output "$OUTPUT/$name.json" \
    >"$OUTPUT/$name.log" 2>&1
}

run_one 0 q06_author_4k_e16_feedback "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 feedback &
run_one 1 q06_author_4k_e16_control  "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 control &
run_one 2 q06_biomed_8k_e16_feedback "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 feedback &
run_one 3 q06_biomed_8k_e16_control  "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 control &
run_one 4 q8_author_4k_e16_feedback "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 feedback &
run_one 5 q8_author_4k_e16_control  "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 control &
run_one 6 q8_author_8k_e8_feedback "$MODEL8" "$CACHE8" qksieve_author_text.txt 8192 8 feedback &
run_one 7 q8_author_8k_e8_control  "$MODEL8" "$CACHE8" qksieve_author_text.txt 8192 8 control &
wait
