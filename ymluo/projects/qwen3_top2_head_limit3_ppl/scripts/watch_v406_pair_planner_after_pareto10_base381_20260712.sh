...
STAMP="${STAMP:-20260712_pair_after_pareto10_base381_v406}"
LABEL="${LABEL:-v406_pair_planner_after_pareto10_base381}"
POLICY="${POLICY:-configs/riskkv_task_policy_v406_pair_planner_after_pareto10_base381_20260712.json}"
...
echo "TRAIN v406 after-pareto pair planner <=10%, base=v381 $(date -Is)"
"$PY" scripts/train_task_gated_pair_planner_v402_20260712.py \
  --base-action policy_v381 \
  --quality-ratio 0.98 \
  --quality-margin 0.03 \
  --kv-limit 0.10 \
  --cal-tol 0.0025 \
  --test-tol 0.0025 \
  --max-kv-multiplier 3.0 \
  --output-dir outputs/riskkv_v19_pair_planner_after_pareto10_base381_v406_20260712 \
  --config-out "$POLICY"
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v406_pair_planner_after_pareto10_base381_20260712_pair_after_pareto10_base381_v406_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v406_pair_planner_after_pareto10_base381_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V406_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v406_pair_planner_after_pareto10_base381_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v406 M100 because M20 gate failed $(date -Is)"
fi
