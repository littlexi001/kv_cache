...
PARENT_RUN=$ROOT/results/20260726_countcap_online_vs_fixed_basis_m20_4gpu
RUN_ROOT=$ROOT/results/20260726_countcap_actual_budget_m4_4gpu
...
while [[ ! -f "$PARENT_RUN/ALL_COMPLETE" ]]; do
  ...
  if ! pgrep -f \
    "launch_countcap_online_vs_fixed_basis_after_spectrum_4gpu_20260726.sh" \
    >/dev/null; then
...
"$PYTHON" src/summarize_countcap_actual_budget_20260726.py \
...
"$PYTHON" - "$RUN_ROOT" "$TASKS" "$SAMPLES" "$METHOD" <<'PY'
import csv
import glob
import json
import sys
...
root, tasks_csv, sample_limit, method = sys.argv[1:]
tasks = {task for task in tasks_csv.split(",") if task}
expected = len(tasks) * int(sample_limit)
...
for model in ("llama31_8b", "qwen3_4b"):
    ...
    assert len(rows) == expected, (model, len(rows), expected)
    assert Counter(row["method"] for row in rows) == {method: expected}
    assert set(row["task"] for row in rows) == tasks
    assert all(row["diagnostics_enabled"].lower() == "true" for row in rows)
    assert all(float(row["attention_link_ratio"]) > 0.0 for row in rows)
    assert all(float(row["selected_history_count_max"]) > 0.0 for row in rows)
    assert all(
        row["sampled_candidate_overflow_fraction"] != "" for row in rows
    )
    assert all(row["sampled_quantile_fallback"] != "" for row in rows)
...
assert all(row["samples"] == expected for row in summary["overall"])
print(f"validated actual-budget audit: two models x {expected} samples")
PY
