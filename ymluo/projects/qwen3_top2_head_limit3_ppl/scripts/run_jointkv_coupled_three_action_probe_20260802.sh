...
OUTPUT="$ROOT/results/20260802_jointkv_coupled_three_action_probe"
...
COMMON=(
  ...
  --adaptive_error_tolerance 0.05
  --adaptive_budget_coupling previous_layer_quantile --adaptive_budget_quantile -1
  --residual_refinement_cost_bits 48 --exact_kv_cost_bits 4096
...
run_one() {
  local gpu=$1 name=$2 model=$3 cache=$4 text=$5 history=$6 eval_tokens=$7 action=$8
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    ...
    --adaptive_action_mode "$action" --output "$OUTPUT/$name.json" \
    >"$OUTPUT/$name.log" 2>&1
}

run_one 0 q06_author_4k_e16_full "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 full_residual &
run_one 1 q06_author_4k_e16_joint "$MODEL06" "$CACHE06" qksieve_author_text.txt 4096 16 joint_cost &
run_one 2 q06_biomed_8k_e16_full "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 full_residual &
run_one 3 q06_biomed_8k_e16_joint "$MODEL06" "$CACHE06" biomed_long_range_facts_hard_compact.txt 8192 16 joint_cost &
run_one 4 q8_author_4k_e16_full "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 full_residual &
run_one 5 q8_author_4k_e16_joint "$MODEL8" "$CACHE8" qksieve_author_text.txt 4096 16 joint_cost &
run_one 6 q8_author_8k_e8_full "$MODEL8" "$CACHE8" qksieve_author_text.txt 8192 8 full_residual &
run_one 7 q8_author_8k_e8_joint "$MODEL8" "$CACHE8" qksieve_author_text.txt 8192 8 joint_cost &
wait
