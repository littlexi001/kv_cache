...
CUDA_VISIBLE_DEVICES=${GPU_V61:-0} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v61_conformal_counterfactual_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v61_conformal_counterfactual_20260709.json \
  > logs/riskkv_v61_conformal_counterfactual_m20_20260709.log 2>&1 < /dev/null &
echo "V61_CONFORMAL_COUNTERFACTUAL_M20_PID=$!"

CUDA_VISIBLE_DEVICES=${GPU_V62:-1} nohup "$PY" src/run_controlled_public_kv_benchmark_v1.py \
  "${common_args[@]}" \
  --output_dir "outputs/riskkv_v62_conformal_counterfactual_qasper_full_m20_20260709" \
  --ours_task_policy_json configs/riskkv_task_policy_v62_conformal_counterfactual_qasper_full_20260709.json \
  > logs/riskkv_v62_conformal_counterfactual_qasper_full_m20_20260709.log 2>&1 < /dev/null &
echo "V62_CONFORMAL_COUNTERFACTUAL_QASPER_FULL_M20_PID=$!"
