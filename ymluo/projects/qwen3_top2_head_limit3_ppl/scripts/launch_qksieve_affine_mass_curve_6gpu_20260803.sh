...
OUTPUT="${OUTPUT:-${ROOT}/results/20260803_qksieve_affine_mass_curve_6gpu_v1}"
...
run_case() {
  ...
  local trace="$3"
  local model="$4"
  for floor in 256 1280; do
    CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -u \
      src/analyze_qksieve_output_risk_budget_20260803.py \
      --trace "${trace}" \
      --output_dir "${OUTPUT}/${name}_floor${floor}" \
      ...
      --risk_bits 4 \
      --focus_mass_ladder \
      >"${OUTPUT}/logs/${name}_floor${floor}.log" 2>&1
    touch "${OUTPUT}/${name}_floor${floor}_COMPLETE"
  done
}
