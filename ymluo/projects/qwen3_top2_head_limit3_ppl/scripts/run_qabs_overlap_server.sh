...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_DIR}/outputs/qabs_overlap_server}"

PREFILL_TOKENS="${PREFILL_TOKENS:-10000}"
EVAL_TOKENS="${EVAL_TOKENS:-1000}"
CHUNK_SIZE="${CHUNK_SIZE:-8}"
EVAL_CHUNK_SIZE="${EVAL_CHUNK_SIZE:-1}"
MODES="${MODES:-qabs8cand3reuse,qabs8cand7reuse,qabs16cand3reuse,qabs16cand7reuse,baseline}"
...
echo "=== qabs overlap prefill=${PREFILL_TOKENS} eval=${EVAL_TOKENS} ==="
...
echo "Overlap summary files:"
find "${out_dir}" -maxdepth 1 -name '*_reuse_overlap_summary.csv' -print
