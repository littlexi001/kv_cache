...
STAMP="${STAMP:-20260712_m100_task_knapsack05_exact_v396}"
LABEL="${LABEL:-v396_m100_task_knapsack05_exact}"
POLICY="${POLICY:-configs/riskkv_task_policy_v396_m100_task_knapsack05_exact_20260712.json}"
...
launch_sync() {
  ...
  env GPUS="$GPUS" LOCK_ROOT="$LOCK_ROOT" GPU_MAX_USED_MB="${GPU_MAX_USED_MB:-2500}" SAMPLES="$samples" LABEL="$LABEL" STAMP="$STAMP" POLICY="$POLICY" TASKS="$ALL_TASKS" bash "$RUNNER" > "$log" 2>&1
}
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v396_m100_task_knapsack05_exact_20260712_m100_task_knapsack05_exact_v396_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v396_m100_task_knapsack05_exact_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(f"V396_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} kv={kv:.2%} speed_full={speed:.2f}x passed={passed}")
PY

if [[ "$(cat outputs/riskkv_v19_v396_m100_task_knapsack05_exact_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v396 M100 because M20 gate failed $(date -Is)"
fi
