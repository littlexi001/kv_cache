...
KV_LIMIT="${KV_LIMIT:?KV_LIMIT is required, for example 0.045}"
LABEL="${LABEL:?LABEL is required, for example v408_task_knapsack045_exact}"
STAMP="${STAMP:?STAMP is required, for example 20260712_task_knapsack045_v408}"
POLICY="${POLICY:?POLICY path is required}"
SUMMARY="${SUMMARY:-outputs/riskkv_v19_${LABEL}_${STAMP}/summary.json}"
...
MAX_KV="${MAX_KV:-$KV_LIMIT}"
MIN_SPEED="${MIN_SPEED:-2.5}"
mkdir -p "$LOG_DIR"

echo "BUILD ${LABEL} kv_limit=${KV_LIMIT} $(date -Is)"
...
max_kv = float(os.environ.get("MAX_KV", os.environ["KV_LIMIT"]))
