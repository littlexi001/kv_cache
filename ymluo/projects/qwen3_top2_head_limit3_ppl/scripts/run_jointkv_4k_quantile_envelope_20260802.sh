...
OUTPUT="$ROOT/results/20260802_jointkv_all_layer_ppl/remote_4k_quantile_envelope"
...
run_one() {
  local gpu=$1 name=$2 quantile=$3
  CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
    "$PYTHON" "${COMMON[@]}" --adaptive_budget_quantile "$quantile" \
