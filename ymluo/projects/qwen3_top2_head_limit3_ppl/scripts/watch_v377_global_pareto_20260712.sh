...
POLICY="${POLICY:-configs/riskkv_task_policy_v377_global_pareto_knapsack_20260712.json}"
STAMP="${STAMP:-20260712_lowkv_global_pareto}"
LABEL="${LABEL:-v377_global_pareto_knapsack}"
...
full_online = 3.0988
full_score = 0.3658
path = Path("outputs/riskkv_v19_v377_global_pareto_knapsack_20260712_lowkv_global_pareto_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v377_global_pareto_knapsack_m100_gate_20260712.flag")
...
passed = score >= 0.38 and 0.01 <= kv <= 0.105 and speed >= 2.5
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(
    f"V377_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} "
    f"kv={kv:.2%} speed_full={speed:.2f}x passed={passed}"
)
PY

if [[ "$(cat outputs/riskkv_v19_v377_global_pareto_knapsack_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v377 M100 because M20 gate failed $(date -Is)"
fi
