...
for IMPL in mask gather sdpa_gather; do
  OUT="/home/fdong/ymluo/projects/qwen3_top2_head_limit3_ppl/outputs/longrange_book_index_sparse_20k_v13_${IMPL}_gqa_smoke"
  mkdir -p "$OUT"
  python -u src/run_longrange_book_index_sparse_eval.py \
    --output_dir "$OUT" \
    "${COMMON_ARGS[@]}" \
    --sparse_attention_impl "$IMPL" \
    > "$OUT/run.log" 2>&1
  cat "$OUT/run.log"
