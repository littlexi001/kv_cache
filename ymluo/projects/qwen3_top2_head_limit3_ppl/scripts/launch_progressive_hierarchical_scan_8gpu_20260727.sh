...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/results/20260727_progressive_hierarchical_scan}"
...
for gpu in "${!labels[@]}"; do
  ...
  nohup env CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" \
    "$ROOT/src/analyze_hierarchical_spectral_quantization_20260727.py" \
    --trace_path "${traces[$gpu]}" \
    --output_dir "$output" \
    --label "$label" \
    --selected_fractions 0.02,0.03,0.04,0.06 \
    --candidate_fractions 0.08,0.12,0.2,0.3,0.5 \
    >"$log" 2>&1 &
