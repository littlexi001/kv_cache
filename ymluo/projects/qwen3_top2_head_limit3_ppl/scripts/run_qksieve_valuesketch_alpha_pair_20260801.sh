...
GPU_IDS="${GPU_IDS:?GPU_IDS must name a physical GPU pair}"
ALPHA="${ALPHA:?ALPHA must lie in [0, 1]}"
TOPICS="${TOPICS:-computer:20260833 medicine:20260832}"
DATASET_CACHE_DIR="${DATASET_CACHE_DIR:-/home/fdong/ymluo/datasets/sklearn}"
VARIANT="${VARIANT:-qksieve_keymse_requestlocal_valuesketch32i4_sampled_k1280}"
...
for entry in ${TOPICS}; do
  topic="${entry%%:*}"
  seed="${entry##*:}"
  output_dir="${RUN_ROOT}/${topic}_alpha${alpha_label}_seed${seed}"
...
touch "${RUN_ROOT}/worker_alpha${alpha_label}_complete"
