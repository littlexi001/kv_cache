...
STAMP="${STAMP:-20260712_task_gated_winner_v391}"
LABEL="${LABEL:-v391_task_gated_winner_router}"
POLICY="${POLICY:-configs/riskkv_task_policy_v391_task_gated_winner_router_20260712.json}"
...
full_score = 0.3658
full_online = 3.0988
path = Path("outputs/riskkv_v19_v391_task_gated_winner_router_20260712_task_gated_winner_v391_m20_bDyn_pDyn/task_results.csv")
flag = Path("outputs/riskkv_v19_v391_task_gated_winner_router_m100_gate_20260712.flag")
...
flag.write_text("1\n" if passed else "0\n", encoding="utf-8")
print(
    f"V391_M20_GATE score={score:.4f} vs_full={score/full_score:.2%} "
    f"kv={kv:.2%} speed_full={speed:.2f}x passed={passed}"
)
PY

if [[ "$(cat outputs/riskkv_v19_v391_task_gated_winner_router_m100_gate_20260712.flag)" == "1" ]]; then
  launch_sync 100
else
  echo "SKIP v391 M100 because M20 gate failed $(date -Is)"
fi
