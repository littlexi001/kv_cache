...
METHODS="full_kv,qksieve_requestlocal_qkbalanced_keymse_wmma_sampled"
...
CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PYTHON}" -u \
  ...
  --device cuda \
  --device_map balanced \
  >"${RUN_ROOT}/logs/keyonly_pair_completion.log" 2>&1

touch "${RUN_ROOT}/KEYONLY_PAIRED_COMPLETE"
