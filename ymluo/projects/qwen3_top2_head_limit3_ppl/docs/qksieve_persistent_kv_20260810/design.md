...
在同一 request-local QK 坐标系下的 Agent 共享长前缀分支中，QKSieve 的 Key
索引与 ValueSketch 可以随精确 KV 一起常驻。分支切换只回退有效长度，不重建
张量；随后追加的新 token 原位覆盖旧后缀。因此，热分支应保持与第一次相同分支
完全一致的生成结果，并获得冻结 Robust 路径的稳态速度。
