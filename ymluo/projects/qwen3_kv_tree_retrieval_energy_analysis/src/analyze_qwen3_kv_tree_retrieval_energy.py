...
def plot_head_rows(
    ...
    for (layer, kv_head, attention_head), rows in sorted(rows_by_head.items()):
        ...
        save_energy_plot(
            ...
            prefix_recent_energy,
            candidate_fraction,
            f"Tree retrieval energy L{layer} KV{kv_head} AttnH{attention_head}",
            path,
            dpi,
        )
        ...
        if smoothing_window > 1:
            ...
            save_energy_plot(
                ...
                f"Tree retrieval energy L{layer} KV{kv_head} AttnH{attention_head} smoothed w={smoothing_window}",
                smooth_path,
                dpi,
            )
            ...
            for layer in layer_indices:
                ...
                for kv_head in kv_head_indices:
                    ...
                    for attention_head in shared_heads:
                        ...
                        oracle = oracle_energy(attention, len(result.candidates), visible) if args.compute_oracle_baseline else ""
