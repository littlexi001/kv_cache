...
def rope_pair_scales(config: ModelConfig, variant: str, layer_index: int, device: torch.device) -> torch.Tensor:
...
def apply_rope(x: torch.Tensor, position_ids: torch.Tensor, inv_freq: torch.Tensor, scales: torch.Tensor) -> torch.Tensor:
    angles = position_ids.float()[:, None] * inv_freq[None, :] * scales[None, :]
...
class Attention(nn.Module):
    ...
    def forward(self, hidden: torch.Tensor) -> torch.Tensor:
        ...
        scales = rope_pair_scales(self.config, self.rope_variant, self.layer_index, hidden.device)
        q = apply_rope(q, position_ids, self.inv_freq, scales)
        k = apply_rope(k, position_ids, self.inv_freq, scales)
...
def main() -> None:
    ...
    parser.add_argument("--variant", required=True, choices=["native", "deep_highfreq_drop", "slow_rope", "smooth_layer_frequency"])
