...
SPARSE_SCRIPT=${SPARSE_SCRIPT:-/home/fdong/ymluo/projects/qwen3_top2_token_mechanism/scripts/run_fused_sampleq_ppl_20260722.sh}
...
run_sparse() {
    mkdir -p "${OUTPUT_ROOT}/sparse"
    bash "${SPARSE_SCRIPT}" \
        "${GPU_LIST}" \
        pca_int4_chunked_logscale16_qkmetric_sampleq_autosplit \
        "${TOPIC}" "${HISTORY_TOKENS}" "${EVAL_TOKENS}" "${OUTPUT_ROOT}/sparse" 48 0.06 \
        > "${OUTPUT_ROOT}/sparse/run.log" 2>&1
}
...
if [[ "${ORDER}" == "full_first" ]]; then
    run_full
    run_sparse
elif [[ "${ORDER}" == "sparse_first" ]]; then
    run_sparse
    run_full
else
    echo "ORDER must be full_first or sparse_first" >&2
    exit 2
fi
