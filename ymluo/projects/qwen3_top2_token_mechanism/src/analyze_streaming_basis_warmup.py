...
def basis_from_samples(key: torch.Tensor, sample_end: int, stride: int) -> torch.Tensor:
    samples = key[:, :sample_end:stride]
    second_moment = torch.einsum("hnd,hne->hde", samples, samples)
    second_moment /= float(samples.shape[1])
    _, eigenvectors = torch.linalg.eigh(second_moment)
    return eigenvectors[..., -64:]
...
def evaluate_trace(path: Path, device: torch.device) -> dict[str, object]:
    ...
    basis_similarity: dict[str, list[float]] = defaultdict(list)
    ...
    for _, records in sorted(records_by_layer.items()):
        ...
        full_basis = basis_from_samples(key, history_count, 32)
        configs = {
            "full_stride32": (history_count, 32),
            "prefix2k_stride8": (min(2048, history_count), 8),
            "prefix4k_stride8": (min(4096, history_count), 8),
            "prefix4k_stride4": (min(4096, history_count), 4),
            "prefix8k_stride8": (min(8192, history_count), 8),
        }
        grouped_query = query.reshape(
            len(records), kv_heads, group_size, query.shape[-1]
        )
        for name, (sample_end, stride) in configs.items():
            basis = (
                full_basis
                if name == "full_stride32"
                else basis_from_samples(key, sample_end, stride)
            )
            ...
            singular_values = torch.linalg.svdvals(
                torch.matmul(full_basis.transpose(-2, -1), basis)
            )
            basis_similarity[name].extend(singular_values.mean(dim=-1).cpu().tolist())
    ...
    return {
        ...
        "mean_principal_cosine": {
            name: summarize(values) for name, values in sorted(basis_similarity.items())
        },
    }
...
def main() -> None:
    ...
    report = {
        "method": "streaming early-frozen PCA basis warmup",
        "traces": [
