...
python "${PROJECT_DIR}/src/eval_rule_permutation.py" \
  --config_dir "${CONFIG_DIR:-${REPO_ROOT}/fdong/Qwen3-0.6B}" \
  --ckpt_file "${CKPT_FILE:-${PROJECT_DIR}/outputs/train/moe-rule-permutation/checkpoints/10000.pth}" \
  --output_path "${OUTPUT_PATH:-}" \
  --load_train_config "${LOAD_TRAIN_CONFIG:-true}" \
  --seed "${SEED:-1234}" \
