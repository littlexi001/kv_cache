...
print_config() {
  cat <<EOF
===== qwen3_moe_structured_language run_train.sh config =====
timestamp=$(date '+%Y-%m-%d %H:%M:%S')
project_dir=${PROJECT_DIR}
repo_root=${REPO_ROOT}
CONFIG_DIR=${CONFIG_DIR:-${REPO_ROOT}/fdong/Qwen3-0.6B}
OUT_DIR=${OUT_DIR:-${PROJECT_DIR}/outputs/train}
RUN_NAME=${RUN_NAME:-structured-language-attn-cluster}
...
python "${PROJECT_DIR}/src/train_structured_language.py" \
  --config_dir "${CONFIG_DIR:-${REPO_ROOT}/fdong/Qwen3-0.6B}" \
  ...
  --debug_vocab_size "${DEBUG_VOCAB_SIZE:-192}" \
