...
OUT_DIR="${OUT_DIR:-${PROJECT_DIR}/outputs/war16k_${STAMP}}"
...
"$PY" -u src/run_per_head_hierarchical_memory.py \
  ...
  --oracle_ratio "${ORACLE_RATIO:-0.02}" \
  --l0_capacity "${L0_CAPACITY:-500}" \
  --l0_recent_tokens "${L0_RECENT_TOKENS:-256}" \
  ...
  --remote_cutoff_tokens "${REMOTE_CUTOFF_TOKENS:-500}" \
...
echo "[per-head-hierarchical-memory] done: $OUT_DIR"
