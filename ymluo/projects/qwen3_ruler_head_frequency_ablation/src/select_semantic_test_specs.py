...
def main() -> None:
    ...
    fixed = next(row for row in rows if row["variant"] == FIXED_CONTROL)
    eligible = [
        row
        for row in rows
        if "_semantic_" in row["variant"]
    ...
    if not eligible:
        raise SystemExit("No semantic-top-k candidate passed the validation constraints")
    selected = eligible[0]
    payload = {
        "selection_rule": (
            "semantic-top-k candidates only; no official sample degradation; "
            "nonnegative official delta on every validation seed; positive mean Gold "
            "NLL improvement; then maximum mean Gold NLL improvement"
        ),
        "source": str(args.validation_summary),
        "selected_semantic": selected["variant"],
