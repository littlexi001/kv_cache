...
LOGDIR=$ROOT/outputs/variable_budget_16k_missing_recovery_20260707_logs
TASKS=(niah_single_1 niah_single_2 niah_multikey_1 niah_multiquery niah_multivalue vt cwe fwe)

IFS=',' read -r -a GPUS <<<"${RECOVERY_GPUS:-0,5,6,7}"
...
launch_missing_for_tag() {
  ...
  for case_id in 0 1; do
    for task in "${TASKS[@]}"; do
      ...
      run_16k_case "$gpu" "$task" "$case_id" "$planner" "$threshold" "$tag" &
      jobs=$((jobs + 1))
...
launch_missing_for_tag bestcal_tail035 "$BESTCAL" 0.35
launch_missing_for_tag conformal_auto "$CONFORMAL" -1
