...
LOGDIR=$ROOT/outputs/variable_budget_ruler_scaling_20260707_logs
...
run_ruler 0 4096 3 "$BESTCAL" 0.35 bestcal_tail035 &
run_ruler 1 8192 3 "$BESTCAL" 0.35 bestcal_tail035 &
run_ruler 2 4096 3 "$CONFORMAL" -1 conformal_auto &
run_ruler 3 8192 3 "$CONFORMAL" -1 conformal_auto &
wait

for tag_case in "bestcal_tail035:$BESTCAL:0.35" "conformal_auto:$CONFORMAL:-1"; do
  IFS=: read -r tag planner threshold <<<"$tag_case"
  for case_id in 0 1; do
    for idx in 0 1 2 3; do
      run_16k_case "$idx" "${TASKS[$idx]}" "$case_id" "$planner" "$threshold" "$tag" &
    done
    wait
