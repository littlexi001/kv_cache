...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Evaluate a static-summary LoRA adapter on heldout texts.")
    parser.add_argument("--output_dir", default="ymluo/projects/learned_hierarchical_summary_memory/outputs/lora_generalization_eval")
    ...
    parser.add_argument("--methods", default="full_raw,recent_only,static_hier,static_sum100,static_sum1000")
    ...
    parser.add_argument("--summary_backend", choices=["heuristic", "learned"], default="learned")
    ...
    parser.add_argument("--learned_summary_seed", type=int, default=2026070310)
...
def load_token_ids(tokenizer: Any, config: Config) -> dict[str, list[int]]:
    ...
    for name, path in zip(config.dataset_names, config.text_paths):
        ...
        token_ids_by_dataset[name] = tokenizer(text, add_special_tokens=False)["input_ids"][: config.max_text_tokens]
    return token_ids_by_dataset
...
def evaluate(
    ...
    rows: list[EvalRow] = []
    model.eval()
    ctx_config = context_config(config)
    for dataset in config.dataset_names:
        ...
        for sample_id, start in enumerate(sample_starts(ids, config)):
            ...
            for method in config.methods:
                prompt = context_for_method(ctx_config, tokenizer, prefix, method, summary_scorer=summary_scorer)
...
def summarize(rows: list[EvalRow], by_dataset: bool) -> list[dict[str, Any]]:
    ...
    for key, items in sorted(grouped.items()):
        ...
        full = full_ref.get(ref_key, {"tokens": avg_tokens, "seconds": avg_seconds})
...
def main() -> None:
    ...
    base_rows = evaluate(model, tokenizer, token_ids_by_dataset, config, "base", device, summary_scorer)
    model = PeftModel.from_pretrained(model, config.adapter_path)
    adapted_rows = evaluate(model, tokenizer, token_ids_by_dataset, config, "adapted", device, summary_scorer)
