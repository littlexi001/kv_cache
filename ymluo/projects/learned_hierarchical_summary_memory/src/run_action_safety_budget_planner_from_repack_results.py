...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Train an action-level safety verifier for variable KV budgets.")
    ...
    parser.add_argument("--safety_thresholds", default="0.5,0.6,0.7,0.8,0.9,0.95")
    parser.add_argument("--seed", type=int, default=2026070795)
...
def action_feature_names(base_names: list[str], compact_actions: list[str]) -> list[str]:
...
def build_action_examples(
    ...
    max_budget = max(vb.action_budget(action) for action in compact_actions)
    out: list[ActionExample] = []
    for case_idx, example in enumerate(examples):
        payload = lookup[(example.source, example.benchmark, example.task, example.case_id)]
        full_score = vb.row_score(payload["full"])
        best_score = vb.row_score(vb.action_row(payload, vb.choose_best(payload)))
        ...
        for rank, action in enumerate(compact_actions):
            ...
            out.append(
                ActionExample(
                    ...
                    improves_full=int(score + 1e-12 >= best_score and best_score > full_score + 1e-12),
...
def normalize_action_features(action_examples: list[ActionExample], train_case_indices: set[int]) -> tuple[list[float], list[float]]:
    train_items = [item for item in action_examples if item.case_index in train_case_indices]
    dim = len(action_examples[0].features)
    mean: list[float] = []
    std: list[float] = []
...
def train_safety_model(
    action_examples: list[ActionExample],
    train_case_indices: list[int],
    test_case_indices: list[int],
    mean: list[float],
    std: list[float],
    config: Config,
) -> tuple[vb.base.MLP, list[dict[str, Any]]]:
    train_set = set(train_case_indices)
    test_set = set(test_case_indices)

    def xy(case_set: set[int]) -> tuple[torch.Tensor, torch.Tensor]:
        selected = [item for item in action_examples if item.case_index in case_set]
...
def action_safe_prob(
    model: vb.base.MLP,
    example: vb.VariableBudgetExample,
    case_index: int,
    action: str,
    action_examples_by_key: dict[tuple[int, str], ActionExample],
    mean: list[float],
    std: list[float],
) -> float:
    item = action_examples_by_key[(case_index, action)]
...
def evaluate_planner(
    ...
    action_lookup = {(item.case_index, item.action): item for item in action_examples}
    for case_idx in case_indices:
        ...
        probs = {
            action: action_safe_prob(model, example, case_idx, action, action_lookup, mean, std)
            for action in compact_actions
            if (case_idx, action) in action_lookup
        }
        for tau in config.safety_thresholds:
            chosen = "full"
            for action in sorted(probs, key=lambda item: (vb.action_budget(item), item)):
                if probs[action] >= tau:
                    chosen = action
                    break
            vb.add_prediction(rows, split, f"safety_tau_{tau:g}", example, target, chosen, lookup)
    return rows
...
def main() -> None:
    ...
    examples, lookup, base_feature_names = vb.build_examples(vb_config)
    ...
    action_examples = build_action_examples(examples, lookup, compact_actions)
    mean, std = normalize_action_features(action_examples, set(train_indices))
    model, history = train_safety_model(action_examples, train_indices, test_indices, mean, std, config)
    rows = evaluate_planner(model, examples, train_indices, "train", compact_actions, action_examples, lookup, mean, std, config)
    rows += evaluate_planner(model, examples, test_indices, "test", compact_actions, action_examples, lookup, mean, std, config)
    summary = vb.summarize(rows)
    names = action_feature_names(base_feature_names, compact_actions)
    write_csv(output_dir / "action_examples.csv", [asdict(item) for item in action_examples])
    ...
    torch.save(
        {
            ...
            "input_dim": len(action_examples[0].features),
            "hidden_dim": config.hidden_dim,
        ...
        output_dir / "action_safety_planner.pt",
    )
    payload = {
        "config": asdict(config),
        "case_examples": len(examples),
        "action_examples": len(action_examples),
        "train_cases": len(train_indices),
        "test_cases": len(test_indices),
        "safe_label_counts": dict(Counter(item.safe_vs_full for item in action_examples)),
    ...
    print(f"saved planner to {output_dir / 'action_safety_planner.pt'}")
