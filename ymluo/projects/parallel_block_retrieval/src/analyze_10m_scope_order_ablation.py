...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze a 10M scope-order ablation.")
...
def main() -> None:
    ...
    ids = sorted(by_order["retrieval_score_order"])
    if len(ids) != args.expected_queries or any(
        set(group) != set(ids) for group in by_order.values()
    ):
        raise RuntimeError("orders must cover the same expected queries")
    quality = {}
    ...
    output = {
        "protocol": {
            "domain": args.domain,
            "real_memory_tokens": "approximately 10M",
            "fixed_retrieval_method": rows[0]["method"],
            "fixed_selected_pages": True,
            "selected_tokens": 512,
            "selection_uses_target": False,
            "scope_order_preserves_first_scope_priority": True,
            "queries": len(ids),
        },
        "quality": quality,
        "versus_retrieval_score_order": {
            order: paired(
                by_order["retrieval_score_order"],
                by_order[order],
                ids,
                samples=args.bootstrap_samples,
                seed=args.seed + index,
            )
            for index, order in enumerate(("scope_old_to_new", "scope_new_to_old"))
        },
        "old_to_new_vs_new_to_old": paired(
            by_order["scope_new_to_old"],
            by_order["scope_old_to_new"],
            ids,
            samples=args.bootstrap_samples,
            seed=args.seed + 10,
        ),
