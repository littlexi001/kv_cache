...
OUTPUT="$PROJECT/outputs/head_prior_debiasing_10m_query480_20260714_v1"
BM25="$PROJECT/outputs/real_longbench_docqa_10m_record480_bm25_20260714_v1"
LOG_ROOT="$PROJECT/outputs/logs/head_prior_debiasing_10m_query480_20260714_v1"
GPU_IDS="${GPU_IDS:-0,1,2,4}"
...
result = {
    "block_ids_exact": bool(np.array_equal(reference["block_ids"], candidate["block_ids"])),
...
result["id_mismatch_slots"] = int(id_mismatch.sum())
result["id_mismatch_fraction"] = float(id_mismatch.mean())
result["all_id_mismatches_have_equal_rank_score"] = bool(
...
if [[ ! -f "$BM25/query_results.csv" ]]; then
  "$PYTHON" src/run_lexical_block_retrieval.py \
