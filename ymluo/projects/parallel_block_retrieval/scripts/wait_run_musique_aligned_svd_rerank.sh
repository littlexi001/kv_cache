...
LOG="${LOG:-${ROOT}/logs/musique_aligned_svd_rerank_v1.log}"
CORPUS="data/musique_official_10m_aligned_400_v2"
CANDIDATES="outputs/musique_official_aligned_10m_lexical_anchor_v1/rows.jsonl"
PROFILE="outputs/musique_official_aligned_10m_sparse_k_top16_v1"
RERANK="outputs/musique_official_aligned_10m_bm25_top16_qk_rerank_v1"
...
"${PYTHON}" src/analyze_train_calibrated_qk_fusion.py \
  --rows_path "${RERANK}/rows.jsonl" \
  --output_path "${RERANK}/train_calibrated_fusion.json" \
  >> "${LOG}" 2>&1

printf '%s aligned SVD32/full128 rerank complete\n' "$(date -Iseconds)" >> "${LOG}"
