...
OUT="${OUT:-$PROJECT/outputs/semantic_common_tail_pilot_qwen3_8b_20260722}"
GPU_ID="${GPU_ID:-7}"
...
nvidia-smi --id="$GPU_ID" --query-gpu=index,uuid,name,memory.total,memory.used,utilization.gpu --format=csv,noheader \
  >"$OUT/gpu_before_launch.txt"

export CUDA_VISIBLE_DEVICES="$GPU_ID"
export TOKENIZERS_PARALLELISM=false
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

if "$PY" -u "$PROJECT/src/run_semantic_common_tail_pilot_8b.py" \
  ...
  --load_in_8bit \
  >"$OUT/logs/experiment.log" 2>&1; then
