...
OUT="${OUT:-$PROJECT/outputs/attention_confidence_qwen3_8b_english_single_token_128k_20260718}"
STEP="${STEP:-500}"
SHORT_MAX="${SHORT_MAX:-49500}"
MAX_LENGTH="${MAX_LENGTH:-128000}"
GLOBAL_MAX_POSITION="${GLOBAL_MAX_POSITION:-130000}"

mkdir -p "$OUT/data" "$OUT/logs"
rm -f "$OUT"/manifest.json "$OUT"/launcher.done "$OUT"/launcher.failed
find "$OUT/data" -maxdepth 1 -type f -name 'length_*.json.tmp' -delete

# Tokenizer-only preflight. This writes the exact selected English words and
# role spans before any GPU process is started.
"$PY" "$PROJECT/src/run_attention_confidence_sweep_8b.py" \
  ...
  --shard_label token_audit \
  --dry_run >"$OUT/token_audit.json"
