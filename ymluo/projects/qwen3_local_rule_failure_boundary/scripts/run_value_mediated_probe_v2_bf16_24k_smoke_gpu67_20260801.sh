...
# Unquantized BF16 causal-closure memory/length smoke at 24K. Physical GPUs 6 and 7 only.
...
OUTPUT=${PROJECT}/outputs/20260801_value_mediated_singleton_v2_bf16_noop_24k_smoke_gpu67
...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=6 "${PYTHON}" "${RUNNER}" \
  "${COMMON[@]}" --seed-start 0 --output-dir "${OUTPUT}/shard_gpu6" \
...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=7 "${PYTHON}" "${RUNNER}" \
  "${COMMON[@]}" --seed-start 1 --output-dir "${OUTPUT}/shard_gpu7" \
...
"${PYTHON}" "${RUNNER}" \
  ...
  >"${OUTPUT}/merged/stdout.log" 2>"${OUTPUT}/merged/stderr.log"
