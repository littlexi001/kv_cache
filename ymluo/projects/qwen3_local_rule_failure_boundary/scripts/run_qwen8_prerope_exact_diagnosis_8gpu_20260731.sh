...
RUN_ROOT="${RUN_ROOT:-${BASE}/outputs/20260731_qwen8_prerope_exact_diagnosis_8gpu}"
LENGTHS="${LENGTHS:-32768,65536}"
VARIANTS="${VARIANTS:-full_rope,rope_top2,semantic_top2_postscore,dual_max_postscore}"
SEED_BASE="${SEED_BASE:-296}"
RATIO="${RATIO:-0.06}"
MAXIMUM_KEEP_TOKENS="${MAXIMUM_KEEP_TOKENS:-1280}"
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  (
    ...
    "${PYTHON}" -u src/run_local_global_rope_probe_8b.py \
      ...
      --maximum-keep-tokens "${MAXIMUM_KEEP_TOKENS}" \
      --local-window 128 \
      --sink-tokens 16 \
