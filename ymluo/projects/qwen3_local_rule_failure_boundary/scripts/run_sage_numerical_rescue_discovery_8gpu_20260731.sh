...
RUN_ROOT="${RUN_ROOT:-${BASE}/outputs/20260731_sage_numerical_rescue_discovery_8gpu}"
LENGTHS="${LENGTHS:-8192,16384,32768,65536}"
VARIANTS="${VARIANTS:-full_rope,rope_top2,local_global_blend25,dual_max_blend25,local_global_postscore,dual_max_postscore,pre_monotone25,dual_monotone25,dual_monotone50,pre_masspreserve25,dual_masspreserve25,dual_masspreserve50}"
...
for gpu in 0 1 2 3 4 5 6 7; do
  output="${RUN_ROOT}/seed${gpu}"
  mkdir -p "${output}"
  if [[ -f "${output}/done.txt" ]]; then
    echo "SKIP completed seed ${gpu}"
    continue
  fi
  (
    ...
    "${PYTHON}" -u src/run_local_global_rope_probe_8b.py \
      ...
      >"${output}/run.log" 2>&1
  ) >"${RUN_ROOT}/logs/seed${gpu}.log" 2>&1 &
  pids+=("$!")
  echo "seed ${gpu}: GPU ${gpu}, PID $!"
done
