...
OUT="${OUT:-$ROOT/outputs/question3_boundary_qwen8b_compare_20260709}"
...
nohup "$PY" -u src/run_local_rule_failure_boundary.py \
  ...
  --distractor_similarities "${DISTRACTOR_SIMILARITIES:-high,conflict}" \
  --rule_gap_tokens "${RULE_GAP_TOKENS:-0,512}" \
  --chain_lengths "${CHAIN_LENGTHS:-1,2,4}" \
  --competitor_counts "${COMPETITOR_COUNTS:-0,4}" \
  --max_cases "${MAX_CASES:-160}" \
  --case_order shuffled \
  --case_order_seed "${CASE_ORDER_SEED:-20260710}" \
  --dtype "${DTYPE:-float16}" \
  --device cuda \
