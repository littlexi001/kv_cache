...
MODEL_LABEL="${MODEL_LABEL:-qwen3_8b_clean_interference}"
OUT="${OUT:-$PROJECT/outputs/clean_interference_qwen8b_20260710}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-5,6,7}" nohup "$PY" -u src/run_local_rule_failure_boundary.py \
...
echo $! > "$OUT"/pid.txt
echo "launched clean interference Qwen3-8B pid=$(cat "$OUT"/pid.txt)"
