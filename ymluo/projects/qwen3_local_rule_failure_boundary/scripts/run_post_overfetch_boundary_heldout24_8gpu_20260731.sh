...
RUN_ROOT="${RUN_ROOT:-${BASE}/outputs/20260731_post_overfetch_boundary_heldout24_8gpu}"
VARIANTS="${VARIANTS:-full_rope,rope_top2,local_global_postscore,post2x_pre_rerank_postscore,post2x_pre_boundary75_postscore,post2x_pre_rerank_masspreserve25}"
SEED_BASE="${SEED_BASE:-160}"
...
for gpu in 0 1 2 3 4 5 6 7; do
  seed=$((SEED_BASE + gpu * SEEDS_PER_GPU))
  output="${RUN_ROOT}/gpu${gpu}_seed${seed}"
  mkdir -p "${output}"
  if [[ -f "${output}/done.txt" ]]; then
    echo "SKIP completed GPU ${gpu}, seeds ${seed}-$((seed + SEEDS_PER_GPU - 1))"
    continue
  fi
  (
  ...
  pids+=("$!")
  echo "GPU ${gpu}: seeds ${seed}-$((seed + SEEDS_PER_GPU - 1)), PID $!"
done
