...
MODEL_LABEL="${MODEL_LABEL:-qwen3_8b_clean_length}"
OUT="${OUT:-$PROJECT/outputs/clean_length_qwen8b_20260710}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-4,5,6,7}" nohup "$PY" -u src/run_local_rule_failure_boundary.py \
  --model_name_or_path "$MODEL" \
  --model_label "$MODEL_LABEL" \
  --output_dir "$OUT" \
  --lengths "${LENGTHS:-1024,8192,16384,32768}" \
...
echo $! > "$OUT"/pid.txt
echo "launched clean length Qwen3-8B pid=$(cat "$OUT"/pid.txt)"
