...
# Unquantized BF16 causal-closure expansion at 16K. Physical GPUs 6 and 7 only.
...
OUTPUT=${PROJECT}/outputs/20260801_value_mediated_singleton_v2_bf16_noop_16k_8seed_gpu67
...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=7 "${PYTHON}" "${RUNNER}" \
  "${COMMON[@]}" --seed-start 4 --output-dir "${OUTPUT}/shard_gpu7" \
