...
OUT_DIR="${OUT_DIR:-${PROJECT_DIR}/outputs/head_function_stability_${STAMP}}"
...
"$PY" -u src/run_head_function_stability.py \
  ...
  --specialization_z_threshold "${SPECIALIZATION_Z_THRESHOLD:-1.0}" \
  --primary_min_log2_enrichment "${PRIMARY_MIN_LOG2_ENRICHMENT:-0.0}" \
  --make_plots "${MAKE_PLOTS:-true}" \
  2>&1 | tee "$OUT_DIR/run.log"

echo "[head-function] done: $OUT_DIR"
