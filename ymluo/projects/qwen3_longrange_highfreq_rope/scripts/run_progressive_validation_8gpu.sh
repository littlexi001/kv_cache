...
RUN_NAME="${1:-progressive_validation_20260807}"
...
BASELINE_RUN="$PROJECT/outputs/candidate_validation_20260807"

mkdir -p "$RUN/specs"
"$PYTHON" "$PROJECT/src/make_progressive_validation_specs.py" \
  --output "$RUN/specs/progressive_validation.json"
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" -u "$BASE/src/run_frequency_sweep.py" \
    ...
    --examples-jsonl "$DATA" \
    --specs-json "$RUN/specs/progressive_validation.json" \
    --output-dir "$shard" \
    --target-length 32768 \
