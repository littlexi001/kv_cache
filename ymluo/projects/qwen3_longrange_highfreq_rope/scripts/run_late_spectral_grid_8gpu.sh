...
RUN_NAME="${1:-late_spectral_grid_20260807}"
...
mkdir -p "$RUN/specs"
"$PYTHON" "$PROJECT/src/make_late_spectral_grid.py" \
  --output "$RUN/specs/grid.json"
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" -u "$BASE/src/run_frequency_sweep.py" \
    ...
    --examples-jsonl "$DATA" \
    --specs-json "$RUN/specs/grid.json" \
    --output-dir "$shard" \
    --sample-ids "$SAMPLES" \
    --target-length 32768 \
