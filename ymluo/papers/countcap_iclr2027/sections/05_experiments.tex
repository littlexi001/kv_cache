...
The reference profile uses $\lambda=0.75$, stride-32 Key samples, eight
prompt-tail Queries, 240-bit plain-scale indices, complete packed-index
top-$k$, and \cref{eq:budget-intro}.  The deployment profile freezes a
model-level QK-balanced/Key-MSE template from disjoint calibration texts and
replaces complete top-$k$ by the capped tail-resolution-calibrated quantile
scan in \cref{eq:frozen-quantile-samples}; its exact-KV consumer and budget are
...
strict smaller screen then runs the exact frozen-template sampled-quantile
deployment path used by the CUDA timing experiment.
...
On 160 strict same-sample pairs, the registered General deployment path
...
.45302.  At the same active-token schedule, General deployment, the
...
absolute macro difference between General and FIER is .00061; this small
...
160 pairs, BinaryPC's released Llama projection retains 99.25\%.  General
...
\begin{table}[t]
\caption{Frozen native-MHA attention path on one RTX~3090.  Full is native
FP16 SDPA without KV replication.  \method includes all selector overhead
and exact sparse attention, but not index construction or ValueSketch.}
\label{tab:mha-main}
\centering
\small
\begin{tabular}{rrrrr}
\toprule
History & Full (ms) & \method (ms) & Speedup & Active/head \\
\midrule
8K   & .1692 & .1386 & 1.22$\times$ & 6.16\% \\
16K  & .3186 & .1946 & 1.64$\times$ & 5.98\% \\
32K  & .6157 & .2312 & 2.66$\times$ & 4.06\% \\
64K  & 1.2031 & .2684 & 4.48$\times$ & 1.96\% \\
128K & 2.3745 & .3810 & 6.23$\times$ & .98\% \\
\bottomrule
\end{tabular}
\end{table}

The frozen timing shape is batch one, 32 Query heads, 32 KV heads, and
$d=128$, with resident FP16 K/V.  CUDA-event measurements use 20 warmup and
80 timed iterations; each row is the median of six algorithmically identical
same-GPU runs spanning three seeds.  The sparse path includes fused Query
projection/quantization, mixed-bit scanning, sampled-threshold candidate
compaction, and exact QK--softmax--AV with eight reduction splits.  Its
realized mixed-bit index averages 5.525\% of FP16 K+V; 5.859\% is the hard
240-bit rate cap.  The frozen 512-sample ceiling improves long-context
selection latency without changing the 8K sample count.  A 16-split variant
reached 7.05$\times$ at 128K but failed output equivalence and is rejected.

\Cref{tab:mha-main} is an attention-subsystem result, not an end-to-end decode
claim.  It excludes prefill and historical index construction, and the MHA
kernel does not include the GQA-4-specific Value-tail correction.  Prior GQA
and whole-model crossover measurements are retained only as audited
diagnostics in \cref{app:system-diagnostics}; they show that short generations
may not amortize initialization even when the attention path is faster.
...
The length gate in \cref{eq:general-profile} is tested on the same 80
LongBench examples from all 16 tasks.  Full scores .404779; always-on rank-16
...
an improvement claim; the result shows that the short-context Robust
perturbation disappears without a task router or Full fallback.  General is
therefore the registered deployment policy; Fast and always-on Robust remain
mechanism ablations.
