...
\input{sections/appendix_analysis_statements}
...
\input{sections/appendix_mechanism_results}
\IfFileExists{data/generated/qksieve_shrinkage_tables.tex}{%
\input{data/generated/qksieve_shrinkage_tables.tex}}{}
\FloatBarrier
\input{sections/appendix_system_diagnostics}

\IfFileExists{data/generated/qksieve_quality_appendix.tex}{%
...
\IfFileExists{data/generated/qksieve_quality_appendix.tex}{%
\clearpage
\section{Additional system protocols}
\label{app:missing}
}{%
\clearpage
...
whether prefill and index construction are paid once or once per decode.  We
report the following timing contracts independently;
numbers from one row cannot be used to fill another.

\begin{table}[h]
\IfFileExists{data/generated/qksieve_h100_tables.tex}{%
\caption{Persistent-KV timing contracts.  The RTX~3090 audit instantiates
32/64K and the matched H100 audit instantiates 64/128K.  Cross-prompt Query
drift is outside the current reuse claim.}}{%
...
Append-only growth & 128-step autoregressive continuation + per-token index append & Long decode/cache growth & TPOT, append correctness \\
\bottomrule
\end{tabular}}
\end{table}

The current directly timed results are in \cref{tab:persistent-kv-main}.
They are medians over three independent process executions of one fixed,
deterministic workload, with process-repetition intervals rather than a claim
about workload or hardware variance.  For 32/64K, median Full/QKSieve warm
latencies are \QKSievePersistentWarmLatencyText\ ms/token, and median index
construction is \QKSievePersistentBuildText\ seconds.  The table separately
reports cold-index timing without dense prefill and cold-E2E timing with dense
prefill; these two quantities must not be interchanged.
...
The extended evaluation grid is $N\in\{16,32,64,128\}$K,
$G\in\{32,64,256,1024\}$, and $R\in\{1,2,4,8,16,32\}$ on repeated-query,
continuous append-only decode, and branching-rollout traces.  It reports latency,
