...
attention，而不是 sparse prefill。最终系统实验将 cold single-use、warm
persistent-cache、shared-prefix branching 和 append-only multi-turn 作为四种
互不混写的计时协议。尚待填写的 workload 网格和 cache 生命周期检查预注册在
\cref{app:persistent-kv-todo}；cold request latency 仍需如实报告为局限，不能与
warm-path 加速混成同一结论。
...
Robust 在 32/64/128K 的索引构建时间为 1.375/1.488/1.839 秒，Fast 为
.768/.774/.743 秒。相对 Full，Robust 的实测 break-even 生成长度为
69/19/10 token，Fast 为 24/9/4 token。若冷请求只生成 64 token并计入构建，
Robust 加速为 .81/1.31/2.15$\times$，Fast 为 1.01/1.64/2.75$\times$。
多轮问答或共享前缀复用只支付一次构建成本，之后得到\cref{tab:mha-decode}的
稳态加速；本文不混合冷请求和 warm reuse 两种口径。
...
原生 MHA 速度合并。因此 Robust 是通用 128K
分支，Fast 保留为低风险长度的速度分支。

我们在同一组覆盖 16 任务的 80 个 LongBench 样本上检验早期 64K 长度门控。
Full 为 .404779；always-on rank-16 Robust
和仅 mean-tail 版本分别保持 98.924\% 与 98.873\%，General 得分 .410725，
保持 101.469\%，paired-bootstrap 区间为 [99.249\%,104.449\%]。本文不把高于
100\% 的点估计解释为提升。该结果暴露了短上下文 Robust 扰动，但 64K 边界
无法由方法数学推出。因此 General 只作为诊断消融保留，always-on Robust 冻结为
主方法；仍需完成其完整同路径 LongBench。

在 256K，Exact-QK
top-1,280 保持 100.24\%，但冻结 240-bit proxy 从 128K 的 98.99\% 降到
80.77\%。Local 坐标配 fixed 240-bit allocation 恢复到 100.44\%，而冻结
坐标配 local bit 只有 89.91\%。这说明根因是坐标外推，而不是精确预算不足
