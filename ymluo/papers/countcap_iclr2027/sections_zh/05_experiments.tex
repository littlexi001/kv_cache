...
\input{data/generated/qksieve_rtx3090_system_rows.tex}
...
审计的 32/64K 切片。
\IfFileExists{data/generated/qksieve_h100_tables_zh.tex}{%
匹配的 H100 64/128K 生命周期结果见\cref{tab:h100-requests}。}{%
其余 H100/128K 网格预注册在\cref{app:persistent-kv-protocol}。}
cold request latency 仍需如实报告为局限，不能与
warm-path 加速混成同一结论。

\IfFileExists{data/generated/qksieve_quality_tables_zh.tex}{%
...
\IfFileExists{data/generated/qksieve_quality_tables_zh.tex}{%
PassageCount 是 LongBench 中唯一相对保持率低于 95\% 的任务：Full 为 .0991，
Robust 为 .0916（92.43\%）。由于 Full 分母本身很小，相对比例会放大绝对
$.0075$ 的下降，因此本文将该弱项单独报告。}{ }
...
Fast/Robust 索引在 32/64/128K 的构建时间为 .768/1.375、.774/1.488 和
.743/1.839 秒，对应 break-even 24/69、9/19 和 4/10 token；直接计时的
64-token 在线加速为 1.01/.81、1.64/1.31 和 2.75/2.15$\times$。结果不包含
dense prefill 和部署前 CUDA JIT，完整表见 \cref{tab:mha-build-break-even}。

\begin{table}[t]
\caption{同一 MHA 模型和 RTX 3090 上直接计时的 persistent-cache 生命周期。
Cold-index 包含 Robust 索引构建和一个 32-token 分支，但不含 dense prefill；
Cold-E2E 包含 prefill。Warm 复用索引；四分支把一次构建均摊给 128 个生成
token。表中为固定确定性工作负载上三个独立进程的中位数和 95\% 进程重复区间。}
\label{tab:persistent-kv-main}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}{rrrrrrr}
\toprule
历史长度 & Cold-index & Cold-E2E & Warm & 四分支 & Append-only & 构建 (s) \\
\midrule
...
32/64K 的 Full/QKSieve warm 延迟中位数分别为
\QKSievePersistentWarmLatencyText\ ms/token；Robust 构建中位数分别为
\QKSievePersistentBuildText\ 秒。按匹配实测值计算
$\lceil I/(D_f-D_s)\rceil$，break-even 分别为
\QKSievePersistentBreakEvenText\ 个生成 token。因此 32K 的单次 32-token
Cold-E2E 如实低于 $1\times$；64K 单次请求大致打平，复用前缀后才获得显著收益。

生命周期审计覆盖全部 32 层、6 个快照和 5 次 rewind，索引指针稳定且重放
完全确定。复用只在 request-local Query 统计不变时成立；新 prompt 需要重建
request-local 索引。这里的 append-only 指同一前缀上连续生成 128 个 token，
并逐 token 追加精确 K/V 与辅助索引；它不包含新用户轮次的一段增量 prefill。
