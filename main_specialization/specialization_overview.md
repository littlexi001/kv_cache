...
要研究 specialization，首先需要定义什么叫“相同 feature”。这个定义在 synthetic 数据和真实语料中不同。
...
因此，这一部分需要回答两个层次的问题：第一，现有 MoE 到底在按什么分发；第二，如果它没有按我们希望的 feature 分发，原因是什么。

### 2.1. 现在的 MoE 如何分发？HRJ，synthetic & real。
已有 synthetic 结论见 [Round4 问题1：Baseline MoE 到底按什么分发](../fdong/inverse_kv_round4_plan.md#问题-1baseline-moe-到底按什么分发)。
...
3. expert assignment 是否主要由 token id、target token、token frequency、position、local context、attention bucket 或 representation cluster 解释；
...
2. 不同 expert 中的 token / context 应当对应不同 feature 区域；
3. expert bucket 不应只是负载均衡后的随机划分，而应当和模型真实使用的预测、检索或语义结构相关；
4. 在真实语料上，理想 expert bucket 应当能通过 logits 相似性、attention retrieval pattern、representation cluster 或语义标签得到解释。
