...
OUTPUT_DIR="${OUTPUT_DIR:-fdong_seq_compress/outputs/answer_access_causal_${TIMESTAMP}}"
mkdir -p "$OUTPUT_DIR"

nohup python3 -u fdong_seq_compress/src/run_answer_access_causal.py \
  ...
  --device "${DEVICE:-mps}" \
  --dtype "${DTYPE:-float16}" \
  --categories "${CATEGORIES:-answer,front,end,other}" \
  --position-ratio "${POSITION_RATIO:-0.01}" \
  --random-seed "${RANDOM_SEED:-2026}" \
